...
<div class="page">
  ...
  <form method="get" action="index.php" name="formSearch" id="formSearch">
    ...
    <input type="hidden" name="op" value="doctors_sale" />
    ...
    <div class="w100pre" style="width: 100%;">
        <table class="tb-type1 nobappointment search left">
          <tbody>
            <tr>
              <th>商品名称</th>
         	  <td><input class="txt-long" type="text" name="doctors_name" value="<?php echo $_GET['doctors_name'];?>" /></td>
         	  <th>店铺名称</th>
         	  <td><input class="txt-long" type="text" name="clic_name" value="<?php echo $_GET['clic_name'];?>" /></td>
         	  <th>平台货号</th>
         	  <td><input class="txt-long" type="text" name="doctors_serial" value="<?php echo $_GET['doctors_serial'];?>" /></td>
         	  </tr>
         	  <tr>
          ...
          <td id="gcategory"><input type="hidden" id="cate_id" name="cate_id" value="" class="mls_id" />
            ...
            <td>
                  ...
                  <option value="day" <?php echo $_REQUEST['search_type']=='day'?'selected':''; ?>>According to the days</option>
                  <option value="week" <?php echo $_REQUEST['search_type']=='week'?'selected':''; ?>>According to the weeks</option>
                  <option value="month" <?php echo $_REQUEST['search_type']=='month'?'selected':''; ?>>According to the months</option>
                </select></td>
  ...
  <div style="text-align:right;">
  	<input type="hidden" id="export_type" name="export_type" data-param='{"url":"<?php echo $output['actionurl'];?>&doctors_name=<?php echo trim($_GET['doctors_name']); ?>&clic_name=<?php echo trim($_GET['clic_name']); ?>&doctors_serial=<?php echo trim($_GET['doctors_serial']); ?>&brand_id=<?php echo intval($_GET['brand_id']); ?>&cate_id=<?php echo intval($_GET['cate_id']); ?>&exporttype=excel"}' value="excel"/>
  ...
  <table class="table tb-type2 nobdb">
    ...
    <?php foreach ($output['doctors_list'] as $k=>$v){?>
      <tr class="hover">
        ...
        <td class="align-center"><?php echo $v['doctors_selltime']==0?date('Y-m-d H:i:s',$v['doctors_addtime']):date('Y-m-d H:i:s',$v['doctors_selltime']); ?></td>
