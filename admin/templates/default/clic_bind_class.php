...
<div class="page">
  ...
  <table class="table tb-type2">
    <thead class="thead">
      <tr class="space">
        <th colspan="15">店铺名称：<?php echo $output['clic_info']['clic_name'];?></th>
      </tr>
    </thead>
    <thead>
      <tr class="thead">
    ...
    <tbody>
      <?php if(!empty($output['clic_bind_class_list']) && is_array($output['clic_bind_class_list'])){ ?>
      <?php foreach($output['clic_bind_class_list'] as $key => $value){ ?>
      <tr class="hover edit">
        ...
        <td class="w60 align-center"><a nctype="btn_del_clic_bind_class" href="javascript:;" data-bid="<?php echo $value['bid'];?>">删除</a></td>
      </tr>
      <?php } ?>
      <?php }else { ?>
      <tr class="no_data">
  ...
  <table class="table tb-type2" >
    ...
    <tbody>
      ...
      <tr class="nobappointment">
        <td class="vatop rowform"><form id="add_form" action="<?php echo urlAdmin('clic', 'clic_bind_class_add');?>" method="post">
            <input name="clic_id" type="hidden" value="<?php echo $output['clic_info']['clic_id'];?>">
            <input id="doctors_class" name="doctors_class" type="hidden" value="">
...
$(document).ready(function(){
    ...
    $('#btn_add_category').on('click', function() {
        ...
        $('#doctors_class').val(category_id);
        $('#add_form').submit();
    });

    // 删除现有类目
    $('[nctype="btn_del_clic_bind_class"]').on('click', function() {
        if(confirm('确认删除？删除后店铺对应分类商品将全部下架')) {
            var bid = $(this).attr('data-bid');
            $this = $(this);
            $.post('<?php echo urlAdmin('clic', 'clic_bind_class_del');?>', {bid: bid}, function(data) {
                 if(data.result) {
    ...
    // 修改分佣比例
    $('span[nc_type="commis_rate"]').inline_edit({act: 'clic',op: 'clic_bind_class_update'});
});
