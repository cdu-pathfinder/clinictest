...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <h3><?php echo $lang['nc_doctors_evaluate']; ?></h3>
      <ul class="tab-base">
        <li><a href="JavaScript:void(0);" class="current"><span><?php echo $lang['admin_evaluate_list'];?></span></a></li>
        <li><a href="index.php?act=evaluate&op=evalclic_list" ><span><?php echo $lang['admin_evalclic_list'];?></span></a></li>
      </ul>
    </div>
  </div>
  ...
  <form method="get" name="formSearch">
    ...
    <input type="hidden" name="op" value="evaldoctors_list" />
    <table class="tb-type1 nobappointment search">
      <tbody>
        <tr>
          <th><label for="doctors_name"><?php echo $lang['admin_evaluate_doctorsname']?></label></th>
          <td><input class="txt" type="text" name="doctors_name" id="doctors_name" value="<?php echo $_GET['doctors_name'];?>" /></td>
          <th><label for="clic_name"><?php echo $lang['admin_evaluate_clicname']?></label></th>
          <td><input class="txt" type="text" name="clic_name" id="clic_name" value="<?php echo $_GET['clic_name'];?>" /></td>
  ...
  <table class="table tb-type2">
    <thead>
      <tr class="thead">
        <th class="w300"><?php echo $lang['admin_evaluate_doctorsname'];?> </th>
        <th><?php echo $lang['admin_evaluate_buyerdesc']; ?></th>
        <th class="w108 align-center"><?php echo $lang['admin_evaluate_frommembername'];?> </th>
        <th class="w108 align-center"><?php echo $lang['admin_evaluate_clicname'];?></th>
        <th class="w72 align-center"><?php echo $lang['nc_handle'];?></th>
      </tr>
    </thead>
    <tbody>
    <?php if(!empty($output['evaldoctors_list'])){?>
      <?php foreach($output['evaldoctors_list'] as $v){?>
      <tr class="hover">
        <td><a href="<?php echo urlclinic('doctors','index',array('doctors_id'=>$v['geval_doctorsid']));?>" target="_blank"><?php echo $v['geval_doctorsname'];?></a></td>
        ...
        <td class="align-center"><?php echo $v['geval_clicname'];?></td>
...
<form id="submit_form" action="<?php echo urlAdmin('evaluate', 'evaldoctors_del');?>" method="post">
