...
<div class="fixed-empty"></div>
    <table class="table tb-type2 nobappointment search">
      <tbody>
      <tr><td>店铺 - <?php echo $output['bill_info']['ob_clic_name'];?>（ID：<?php echo $output['bill_info']['ob_clic_id'];?>） <?php echo $output['bill_info']['ob_create_month'];?>期 结算单&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      ...
      <a class="btns" target="_blank" href="index.php?act=bill&op=export_step1&bill_id=<?php echo $_GET['bill_id'];?>"><span><?php echo $lang['nc_exposrt'];?>导出结算明细</span></a>
        ...
        <tr><td><?php echo $lang['appointment_time_from'];?>结算单号：<?php echo $output['bill_info']['ob_no'];?></td></tr>
      ...
      <td>平台应付金额：<?php echo number_format($output['bill_info']['ob_appointment_real_totals']-$output['bill_info']['ob_commission_totals'],2,'.','');?> = <?php echo $output['bill_info']['ob_appointment_real_totals'];?> (实收订单金额) - <?php echo $output['bill_info']['ob_commission_totals'];?> (佣金)</td>
      </tr>
  ...
  <form method="get" action="index.php" name="formSearch" id="formSearch">
    ...
    <input type="hidden" name="op" value="show_clic_month_appointment" />
    <input type="hidden" name="bill_id" value="<?php echo $_GET['bill_id'];?>" />
    <table class="tb-type1 nobappointment search">
      <tbody>
        <tr>
        <th><label for="add_time_from"><?php echo $lang['appointment_time_from'];?>订单类型</label></th>
          <td>
          ...
          <th><label for="add_time_from"><?php echo $lang['appointment_time_from'];?>下单时间</label></th>
  ...
  <table class="table tb-type2 nobdb">
    <thead>
      <tr class="thead">
        <th class="align-center"><?php echo $lang['appointment_number'];?>订单编号</th>
        <th class="align-center"><?php echo $lang['clic_name'];?>订单金额</th>
        <th class="align-center"><?php echo $lang['buyer_name'];?>商品金额</th>
        <th class="align-center"><?php echo $lang['appointment_time'];?>运费</th>
        ...
        <?php }?>
        <th class="align-center"><?php echo $lang['appointment_state'];?>下单日期</th>
    ...
    <tbody>
      ...
      <tr class="hover">
        <td class="align-center"><?php echo $appointment['appointment_sn'];?></td>
        <td class="align-center"><?php echo number_format($appointment['doctors_amount']+$appointment['shipping_fee'],2,'.','');?></td>
        <td class="align-center"><?php echo $appointment['doctors_amount'];?></td>
        <td class="align-center"><?php echo $appointment['shipping_fee'];?></td>
        <?php if ($_GET['query_type'] != 'refund'){?>
        <td class="align-center"><?php echo number_format($appointment['doctors_amount']*$appointment['commission_rate']/100,2);?></td>
        <?php }else{?>
        <td class="align-center">全部退款</td>
        <?php }?>
        <td class="align-center"><?php echo date('Y-m-d',$appointment['add_time']);?></td>
        <td class="align-center"><?php echo date('Y-m-d',$appointment['finnshed_time']);?></td>
        <td class="align-center">
        <?php
	        if (!empty($appointment['voucher_id'])) echo '代金券:'.$appointment['voucher_price'];
	        if (!empty($appointment['group_id'])) echo '[团购]';
	        if (!empty($appointment['xianshi_id'])) echo '[限时]';
	        if (!empty($appointment['mansong_id'])) echo '[满送]';
	        if (!empty($appointment['bundling_id'])) echo '[组合]';
        ?>
        </td>
        <td class="align-center"><?php echo $appointment['buyer_name'];?></rd>
        <td>
        <a href="index.php?act=appointment&op=show_appointment&appointment_id=<?php echo $appointment['appointment_id'];?>"><?php echo $lang['nc_view'];?></a>
        </td>
      </tr>
      <?php }?>
      <?php }else{?>
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
    	$('input[name="op"]').val('show_clic_month_appointment');$('#formSearch').submit();
    });
});
</script>
