...
elseif(strpos($x, '*') === false){$e = array_flip(explode(',', $x));}
 ...
 if(isset($x[1])){
  preg_match_all('`(?:^|-|\+)[^\-+]+?(?=-|\+|$)`', $x, $m, PREG_SET_appointment);
...
if($depTr){
 ...
 foreach($a as $k=>$v){
  ...
  if($k == 'align'){
  ...
  }elseif($k == 'bappointment'){
   unset($a['bappointment']); $c[] = "bappointment: {$v}px";
  }elseif($k == 'bappointmentcolor'){
   unset($a['bappointmentcolor']); $c[] = 'bappointment-color: '. $v;
  }elseif($k == 'clear'){
   ...
   unset($a['noshade']); $c[] = 'bappointment-style: none; bappointment: 0; background-color: gray; color: gray';
  }elseif($k == 'nowrap'){
