...
$(document).ready(function(){
    ...
    $("#comment_message").microclinic_publish({
        ...
        $.post(url_comment_save, $("#add_form").serialize(),
            function(data){
                if(data.result == 'true') {
                    ...
                    $("#comment_count").microclinic_count({type:'+'});
                } else {
...
<div id="widgetcommenttitle" class="widget-comment-title">
  <h3><?php echo $lang['microclinic_comment_title'];?></h3>
...
<form id="add_form">
  ...
  <div><a id="btn_show_smilies" href="javascript:void(0)" class="smile"><?php echo $lang['microclinic_comment_smiles'];?></a> 
    <!-- 站外分享 -->
    <?php require('widget_share.php');?>
  <input id="btn_publish_comment" class="input-button" type="button" value="<?php echo $lang['microclinic_text_publish'];?>" />
  </div>
</form>
</dd>
<div class="clear">&nbsp;</div>
...
<?php } else { ?>
<div class="login"> <?php echo $lang['no_login'];?> <a href="<?php echo clinic_SITE_URL.'/index.php?act=login&ref_url='.getRefUrl();?>"><?php echo $lang['nc_login'];?></a> </div>
<?php } ?>
