...
<div class="commend-doctors">
  <div class="commend-doctors-info">
    <div class="user">
      ...
      <dl>
        <dt> <a href="<?php echo MICROclinic_SITE_URL;?>/index.php?act=home&member_id=<?php echo $output['detail']['commend_member_id'];?>" target="_blank" nctype="mcard" data-param="{'id':<?php echo $output['detail']['member_id'];?>}"> <?php echo $output['detail']['member_name'];?></a><?php echo $lang['microclinic_personal_commend'];?><span class="add-time"><?php echo date('Y-m-d',$output['detail']['commend_time']);?></span></dt>
        <dd><i></i>
    ...
    <div class="doctors">
      <div class="handle-bar" id="personal_buy">
        <div class="buy-btn">
          ...
          <a href="javascript:void(0)"><span><?php echo $lang['microclinic_text_buy'];?></span><i></i></a>
          <?php $buy_array = unserialize($output['detail']['commend_buy']);?>
          <div class="buy-info">
            <?php foreach($buy_array as $val) { ?>
            <dl>
              <dt class="doctors-pic"><img src="<?php echo $val['image'];?>" alt="<?php echo $val['title'];?>" title="<?php echo $val['title'];?>" /></dt>
              <dd><a href="<?php echo $val['link'];?>" class="doctors-name" title="<?php echo $val['title'];?>" target="_blank"><?php echo $val['title'];?></a>
                <p class="doctors-price"><?php echo $lang['currency'].$val['price'];?></p>
              </dd>
            </dl>
            <?php } ?>
          </div>
        ...
        <div class="buttons"><a nc_type="microclinic_like" like_id="<?php echo $output['detail']['personal_id'];?>" href="javascript:void(0)" class="like"><i></i><em><?php echo $output['detail']['like_count']<=999?$output['detail']['like_count']:'999+';?></em></a><a id="btn_sns_share" href="javascript:void(0)" class="share"><i></i><?php echo $lang['microclinic_text_share'];?><em></em></a></div>
      </div>
      <?php $personal_image_array = getMicroclinicPersonalImageUrl($output['detail']);?>
      <?php if(!empty($personal_image_array)) { ?>
      <?php foreach($personal_image_array as $val) { ?>
      <div class="pic"> <img class="lazy" src="<?php echo MICROclinic_TEMPLATES_URL;?>/images/loading.gif" data-src="<?php echo $val;?>" /> </div>
      <?php } ?>
      <?php } ?>
  ...
  <div id="widget_comment_title" class="commend-doctors-sidebar">
