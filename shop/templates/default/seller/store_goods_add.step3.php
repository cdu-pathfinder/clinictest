...
<form method="post" id="goods_image" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_image'); } else { echo urlShop('store_goods_add', 'save_image');}?>">
  ...
  <div class="ncsc-form-goods-pic">
      ...
      <div class="ncsc-goodspic-list">
        ...
        <ul nctype="ul<?php echo $value['sp_value_id'];?>">
          ...
          <li class="ncsc-goodspic-upload">
            ...
            <div class="show-sort">Sorting:<input name="img[<?php echo $value['sp_value_id'];?>][<?php echo $i;?>][sort]" type="text" class="text" value="<?php echo intval($output['img'][$value['sp_value_id']][$i]['goods_image_sort']);?>" size="1" maxlength="1">
            </div>
        ...
        <div class="ncsc-select-album">
          <a class="ncsc-btn" href="index.php?act=store_album&op=pic_list&item=goods_image&color_id=<?php echo $value['sp_value_id'];?>" nctype="select-<?php echo $value['sp_value_id'];?>"><i class="icon-picture"></i>from picture space</a>
    ...
    <li>3. Each color can upload up to 5 images or pictures from space, select an existing picture upload images will also be stored in the clinic after space for other use.</li>
