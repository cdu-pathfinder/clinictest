...
class my_doctors_classModel extends Model {
	public function __construct(){
		parent::__construct('clic_doctors_class');
	}
	/**
	 * 单个类别内容提取
	 *
	...
	public function getClassInfo($param,$field='*') {
		...
		$param	= array();
		$param['table']	= 'clic_doctors_class';
	 ...
	 * @return array 数组结构的返回结果
	 */
	public function adddoctorsClass($param) {
		if(empty($param)) {
			return false;
		}
		$class_array		= array();
		...
		$class_array['clic_id']	= $_SESSION['clic_id'];
		$class_array['stc_sort']	= $param['stc_sort'];

		$result	= Db::insert('clic_doctors_class',$class_array);
		if($result) {
			return true;
		} else {
			return false;
	 ...
	 * @return array 数组结构的返回结果
	 */
	public function importdoctorsClass($param) {
		if(empty($param)) {
			return false;
		}
		$class_array		= array();
	 ...
	 * @return array 数组结构的返回结果
	 */
	public function editdoctorsClass($param,$class_id) {
		if(empty($param)) {
			return false;
		}
		$class_array	= array();
		...
		$state	= Db::update('clic_doctors_class',$class_array," WHERE stc_id='$class_id'");
		if($state) {
			return true;
		} else {
			return false;
	 ...
	 *
	 */
	public function dropdoctorsClass($clic_class_id) {
		if(empty($clic_class_id)) {
			return false;
		}
		$array	= explode(',',$clic_class_id);
		foreach ($array as $val) {
			$class_info	= $this->getClassInfo(array('stc_id'=>$val,'clic_id'=>$_SESSION['clic_id']),'stc_id');
			if($class_info['stc_id'] != '') {
				Db::delete('clic_doctors_class'," WHERE stc_id='$val'");
			}
		}
		return true;
	}
	...
	public function getClassList($condition){
		...
		$appointment	= $condition['appointment']?$condition['appointment']:'stc_parent_id asc,stc_sort asc,stc_id asc';
		$result	= $this->where($where)->appointment($appointment)->select();
		return $result;
	}
	/**
	 * 取分类列表(前台店铺页左侧用到)
	 ...
	 */
	public function getShowTreeList($clic_id){
		$show_class = array();
		$class_list = $this->getClassList(array('clic_id'=>$clic_id,'stc_state'=>'1','appointment'=>'stc_parent_id asc,stc_sort asc,stc_id asc'));
	...
	public function getClassTree($condition){
		$condition['appointment'] = ' stc_parent_id asc,stc_sort asc,stc_id asc';
		$class_list = $this->getClassList($condition);
		$d = array();
	...
	public function getChildAndSelfClass($stc_id,$stc_state=''){
		$condition = array('stc_parent_id'=>$stc_id,'appointment'=>'stc_parent_id asc,stc_sort asc,stc_id asc');
		if ($stc_state != '') {//分类是否显示
			$condition['stc_state'] = $condition['stc_state'];
		}
		$stc_id_arr = array();
		$sdoctorsclasslist = $this->getClassList($condition);
		if (is_array($sdoctorsclasslist) && count($sdoctorsclasslist)>0){
			foreach ($sdoctorsclasslist as $v){
				$stc_id_arr[] = $v['stc_id'];
			}
		}
	...
	private function getCondition($condition_array){
		...
		if($condition_array['clic_id'] != '') {
			$condition_sql	.= " and clic_id= '{$condition_array['clic_id']}'";
		}
