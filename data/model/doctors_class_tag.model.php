...
class doctors_class_tagModel extends Model{
    public function __construct() {
        parent::__construct('doctors_class_tag');
    }
     ...
     * @return boolean
     */
    public function deldoctorsClassTag($condition) {
	...
	public function getTagList($condition = array() , $page = '', $field='*'){
		...
		$param = array();
		$param['table'] = 'doctors_class_tag';
		$param['field'] = $field;
		$param['where'] = $condition_str;
		$param['appointment'] = $condition['appointment'] ? $condition['appointment'] : 'gc_tag_id asc';
	...
	public function tagAdd($param){
		...
		return Db::query("insert into `".DBPRE."doctors_class_tag` (`gc_id_1`,`gc_id_2`,`gc_id_3`,`gc_tag_name`,`gc_tag_value`,`gc_id`,`type_id`) values ".$condition_str);
	}
	...
	public function clearTag(){
		return Db::query("TRUNCATE TABLE `".DBPRE."doctors_class_tag`");
	}
	...
	public function updateTag($param){
		...
		if (is_array($param)){
			...
			$result = Db::update('doctors_class_tag',$tmp,$where);
			return $result;
		}else {
			return false;
		}
	...
	public function delTagByIds($id){
		if(!empty($id)) {
			return Db::delete('doctors_class_tag',' gc_tag_id in ('.$id.')');
		}else{
			return false;
		}
	}
	...
	public function delByCondition($condition){
		return Db::delete('doctors_class_tag', $this->_condition($condition));
	}
	...
	public function addOneTag($param){
		...
		if (is_array($param)){
			...
			$result = Db::insert('doctors_class_tag',$tmp);
			return $result;
		}else {
			return false;
		}
