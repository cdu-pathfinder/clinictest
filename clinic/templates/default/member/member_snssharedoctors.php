...
<div class="feededitor">			
  <form method="post" action="index.php?act=member_snsindex&op=sharedoctors<?php if($_GET['irefresh'] == 1){?>&irefresh=1<?php }?>" id="sharedoctors_form">
    <input type="hidden" name="form_submit" value="ok"/>
    <!-- 商品图片start -->
    <?php if (!empty($output['doctors_list'])){?>
    <ul id="mycarousel-s" class="snsdoctorsimglist jcarousel-skin-tango">
      <?php foreach ($output['doctors_list'] as $v) {?>
      <li style="float: left;"><a href="javascript:void(0);" value="<?php echo $v['doctors_id'] ?>"> <span class="thumb size90"><?php if($v['appointment']){?><i class="b"></i><?php }?><?php if($v['favorites']){?><i class="f"></i><?php }?> <img title="<?php echo $v['doctors_name']?>" src="<?php echo thumb($v, 240);?>" onload="javascript:DrawImage(this,90,90);"/>
        <p class="extra"><?php echo $lang['sns_selected'];?></p>
        </span> </a></li>
      <?php }?>
    </ul>
    <?php } else{?>
    <div class="sns-norecord"><?php echo $lang['sns_sharedoctors_notbuydoctors'];?></div>
    <?php }?>
    <input type="hidden" id="choosedoctorsid" name="choosedoctorsid" value="<?php echo intval($output['doctors_list'][0]['doctors_id']);?>" />
    <div class="error"></div>
    ...
    <!-- 商品图片end -->
    <div class="p10">
      <textarea placeholder="<?php echo $lang['sns_sharedoctors_contenttip'];?>" name="comment" id="content_sgweibo" resize="none"></textarea>
      ...
      <div id="sg_seccode" class="seccode">
        ...
        <img src="" title="<?php echo $lang['wrong_checkcode_change'];?>" name="codeimage" bappointment="0" onclick="this.src='index.php?act=seccode&op=makecode&nchash=<?php echo $output['nchash'];?>&t=' + Math.random()"/>
...
$(function(){
	...
	//初始化选择的商品
	$(".snsdoctorsimglist").find('a').eq(0).addClass("selected");
	//商品列表绑定事件
	$(".snsdoctorsimglist").find('a').bind("click",function(){
		$(".snsdoctorsimglist").find('a').removeClass('selected');
		$(this).addClass("selected");
		var gid = $(this).attr('value');
		$("#choosedoctorsid").val(gid);
	});
	//分享商品评论字符个数计算
	...
	$('#sharedoctors_form').validate({
	    ...
	    messages : {
	    	choosedoctorsid:{
	    		min:'<?php echo $lang['sns_sharedoctors_choose'];?>'
	    	},
	    	comment : {
