...
<div id="lazymodule">
  ...
  <ul class="fd-list">
    ...
    <div class="fd-aside"> <span class="thumb size60"><i></i> <a href="index.php?act=clic_snshome&sid=<?php echo $val['strace_clicid'];?>" target="_blank"> <img onload="javascript:DrawImage(this,60,60);" src="<?php echo getclicLogo($val['strace_cliclogo']);?>"> </a> </span> </div>
      <dl class="fd-wrap">
        <dt>
          <h3><a href="index.php?act=clic_snshome&sid=<?php echo $val['strace_clicid'];?>" target="_blank"><?php echo $val['strace_clicname'];?></a><?php echo $lang['nc_colon'];?></h3>
          <h5><?php echo parsesmiles($val['strace_title']);?></h5>
          <?php if ($_SESSION['clic_id'] == $val['strace_clicid']){?>
          ...
          <div id="forward_<?php echo $val['strace_id'];?>" style="display:none;">
            <div class="forward-widget">
              <div class="forward-edit">
                <form id="forwardform_<?php echo $val['strace_id'];?>" method="post" action="index.php?act=clic_snshome&op=addforward">
  ...
  <?php }else{?>
  <div class="sns-norecord"><?php echo $lang['clic_sns_collect_null'];?></div>
  <?php }?>
</div>
<!-- 表情弹出层 -->
