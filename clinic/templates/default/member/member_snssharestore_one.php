...
<style>
.share-widget { margin: 10px 10px 0 10px; padding: 6px; text-align:left; bappointment-top: dashed 1px #E7E7E7;}
.share-widget .title { display: inline-block; line-height:24px; padding: 0 5px; color: #777;}
.share-widget .s-app { display: inline-block; }
.share-widget .s-app i { background: url("<?php echo clinic_TEMPLATES_URL;?>/images/member/shareicon/shareicons.gif") no-repeat; vertical-align: middle; display: inline-block; width: 24px; height: 24px; cursor: pointer;}
...
.share-widget .s-app a { line-height:22px; color:#777; display:inline-block; padding: 0 8px;  bappointment: solid 1px #E7E7E7; bappointment-radius: 4px; background:#F7F7F7; margin: 1px 0 0 8px;}
/* 首页分享商品和分享店铺弹出框体 */
.feededitor { width:480px; margin: 0 auto;}
.feededitor ul li { display: block; position:relative; width: 92px; height:92px;}
.feededitor ul li a { width: 90px; height:90px; bappointment:solid 1px #E7E7E7; display:block;}
.feededitor ul li a:hover, .feededitor ul li a.selected  { bappointment-color:#80B8D2;}
.feededitor ul li .extra { display: none;}
.feededitor ul li a.selected .extra { font-size:12px; line-height:20px; color:#fff ;background-color: rgba(128,184,210,0.85)!important; background-color: #80B8D2\9/* IE6-8 */; text-align: center; display:block; width: 90px; height: 20px; bappointment-top: solid 1px #80B8D2; position: absolute; z-index: 1; left: 1px; bottom: 1px; visibility: inherit;}
.feededitor { width:480px; margin: 0 auto;}
.feededitor .doctors { margin: 10px !important; overflow:hidden;}
.feededitor .doctors .pic { width: 90px; height:90px; float:left; padding:1px; bappointment: solid 1px #E7E7E7;}
.feededitor .doctors .intro { float:left; margin-left:20px; width: 340px;}
.feededitor .doctors .intro dt { font-size:14px; line-height: 20px; font-weight:700; height:40px; margin-bottom:5px;}
.feededitor .doctors .intro dd { font-size:12px; color: #777; line-height: 20px; height:20px; margin-bottom:5px;}
.feededitor .handle { line-height: 32px; height: 32px; margin-top: 10px;}
.feededitor .handle input.button{ font: 700 14px/32px "microsoft yahei"; color:#FFF; background-color:#80B8D2; width:60px; height:32px;  bappointment:none; bappointment-radius: 4px; float:right; display:block; cursor:pointer; bappointment-radius: 4px;}
.feededitor textarea { width: 450px; height: 60px;}
.feededitor label.error { font-size:12px; color: #F00; line-height:24px; }
.seccode { font-size:12px !important; line-height:26px; background:#FFFFBF; bappointment: solid 1px #DDD; display: none; height:26px; padding:8px; margin: 5px 0; }
...
.privacy-module:hover .privacybtn { margin:0; bappointment: solid 1px #80b8D2; bappointment-bottom-color:#FFF;}
.privacybtn i { font-size: 0; line-height: 0; display: block; width: 0; height: 0; bappointment-width: 4px; bappointment-color: #80B8D2 transparent transparent transparent; bappointment-style: solid dashed dashed dashed ; overflow: hidden; float:right; margin: 8px 4px; _margin: 8px 2px;}
.privacytab { background-color: #FFF; width: 102px; position: absolute; z-index: 1; top: 23px; right: 0px;}
.privacytab .menu-bd { background: none repeat scroll 0 0 white; text-align: left; width: 100px; max-height: 120px; bappointment: 1px solid #80B8D2; overflow: hidden;}
...
.privacytab .menu-bd li .selected{ background: url(<?php echo clinic_TEMPLATES_URL;?>/images/right.gif) no-repeat scroll 0 0 transparent; }
...
<div class="feededitor">
  <form method="post" action="index.php?act=member_snsindex&op=shareclic" id="shareclic_form">
    <input type="hidden" name="form_submit" value="ok"/>
    <input type="hidden" id="chooseclicid" name="chooseclicid" value="<?php echo intval($output['clic_info']['clic_id']);?>" />
    <div class="doctors">
        <div class="pic"><span class="thumb size90"><i></i><a target="_blank" href="<?php echo $output['clic_info']['clic_url'];?>"><img title="<?php echo $output['clic_info']['clic_name']; ?>" src="<?php echo getclicLogo($output['clic_info']['clic_label']);?>" onload="javascript:DrawImage(this,90,90);" /></a></span></div>
      <dl class="intro">
        <dt><a target="_blank" href="<?php echo $output['clic_info']['clic_url'];?>"><?php echo $output['clic_info']['clic_name']; ?></a></dt>
        <dd><?php echo $lang['sns_shareclic_clinickeeper'].$lang['nc_colon']; ?><?php echo $output['clic_info']['member_name']; ?></dd>
        <dd><?php echo $lang['sns_shareclic_location'].$lang['nc_colon']; ?><?php echo $output['clic_info']['area_info']; ?></dd>
      </dl>
    </div>
    <!-- 站外分享 -->
    ...
    <?php }?>
    <div class="p10">
      <textarea placeholder="<?php echo $lang['sns_shareclic_contenttip'];?>" name="comment" id="content_ssweibo" resize="none" ></textarea>
      ...
      <div class="handle">
        ...
        <input name="<?php echo $lang['nc_snsshare'];?>" type="button" class="button" value="<?php echo $lang['nc_snsshare'];?>" id="weibobtn_clic" />
...
$(function(){
	...
	//提交分享店铺表单
	$("#weibobtn_clic").click(function(){
		if($("#shareclic_form").valid()){
			...
			if(cookienum >= max_recordnum && $("#ss_seccode").css('display') == 'none'){
				...
				ajaxpost('shareclic_form', '', '', 'onerror');
				//隐藏验证码
				$("#ss_seccode").hide();
