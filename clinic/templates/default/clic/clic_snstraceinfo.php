  ...
  <li nc_type="stracerow_<?php echo $output['strace_info']['strace_id']; ?>">
  	<div class="fd-aside">
		<span class="thumb size60"><i></i>
			<a href="index.php?act=clic_snshome&sid=<?php echo $output['strace_info']['strace_clicid'];?>" target="_blank">
				<img src="<?php echo getclicLogo($output['strace_info']['strace_cliclogo']);  ?>" onload="javascript:DrawImage(this,60,60);">
			</a>
		</span>
      </div>
    <dl class="fd-wrap">
      <dt>
        <h3><a href="index.php?act=clic_snshome&sid=<?php echo $output['strace_info']['strace_clicid'];?>" target="_blank"><?php echo $output['strace_info']['strace_clicname'];?></a><?php echo $lang['nc_colon'];?></h3>
        <h5><?php echo $output['strace_info']['strace_title'];?></h5>
  		<?php if ($_SESSION['clic_id'] == $output['strace_info']['strace_clicid']){?>
	  ...
	  <dd>
		<span class="doctors-time fl"><?php echo date('Y-m-d H:i',$output['strace_info']['strace_time']);?></span>
		<span class="fr">
		...
		<div id="forward_<?php echo $output['strace_info']['strace_id'];?>" style="display:none;">
			<div class="forward-widget">
				<div class="forward-edit">
					<form id="forwardform_<?php echo $output['strace_info']['strace_id'];?>" method="post" action="index.php?act=clic_snshome&op=addforward">
...
$(function(){
    $("#tracereply_<?php echo $output['strace_info']['strace_id'];?>").load('index.php?act=clic_snshome&op=commentlist&id=<?php echo $output['strace_info']['strace_id'];?>').show();
    $('.demo').ajaxContent({
