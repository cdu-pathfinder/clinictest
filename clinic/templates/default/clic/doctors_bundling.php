...
<div class="ncs-bundling-container">
  ...
  <?php if(!empty($output['b_doctors_array'][$val['id']]) && is_array($output['b_doctors_array'][$val['id']])){$i++;?>
  <div class="ncs-bundling-list">
  <ul>
    <h3><?php echo $lang['bundling'].$i.$lang['nc_colon'].$val['name'];?></h3>
    <?php ksort($output['b_doctors_array'][$val['id']]);foreach($output['b_doctors_array'][$val['id']] as $v){?>
    <li>
      <dl>
        <dt class="doctors-name" title="<?php echo $v['name'];?>"><a href="<?php echo urlclinic('doctors', 'index', array('doctors_id' => $v['id']));?>" target="block"><?php echo $v['name'];?></a></dt>
        <dd class="doctors-thumb"><a href="<?php echo urlclinic('doctors', 'index', array('doctors_id' => $v['id']));?>" target="block"><img src="<?php echo $v['image'];?>" title="<?php echo $v['name'];?>" alt="<?php echo $v['name'];?>" onload="javascript:DrawImage(this,100,100);" /></a></dd>
        <dd class="doctors-price">Platform Price<?php echo $lang['nc_colon'].$lang['currency'].$v['clinic_price'];?></dd>
