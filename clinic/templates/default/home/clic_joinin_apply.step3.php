...
$(document).ready(function(){
    ...
    jQuery.validator.addMethod("seller_name_exist", function(value, element, params) { 
        ...
        $.ajax({  
            type:"GET",  
            url:'<?php echo urlclinic('clic_joinin', 'check_seller_name_exist');?>',  
            async:false,  
    ...
    $('#btn_add_category').on('click', function() {
        var tr_category = '<tr class="clic-class-item">';
        var category_id = '';
        var category_name = '';
        var class_count = 0;
        var validation = true;
        ...
        if(validation) {
            ...
            tr_category += '<input name="clic_class_ids[]" type="hidden" value="' + category_id + '" />';
            tr_category += '<input name="clic_class_names[]" type="hidden" value="' + category_name + '" />';
            tr_category += '</tr>';
            $('#table_category').append(tr_category);
            $('#gcategory').hide();
            $('#btn_select_category').show();
            select_clic_class_count();
        } else {
    ...
    // 统计已经选择的经营类目
    function select_clic_class_count() {
        var clic_class_count = $('#table_category').find('.clic-class-item').length;
        $('#clic_class').val(clic_class_count);
    }
    ...
    $('#btn_apply_clic_next').on('click', function() {
        if($('#form_clic_info').valid()) {
            $('#form_clic_info').submit();
        }
    });
});
</script>
...
<div id="apply_clic_info" class="apply-clic-info">
  <div id="apply_company_info" class="apply-company-info">
    ...
    <form id="form_clic_info" action="index.php?act=clic_joinin&op=step4" method="post" >
      <table bappointment="0" cellpadding="0" cellspacing="0" class="all">
        <thead>
          <tr>
        ...
        <tr>
          ...
          <tr>
            ...
            <td><input name="clic_name" type="text" class="w200"/>
              <span></span>
          ...
          <tr>
            ...
            <td><select name="sg_id" id="sg_id">
                ...
                <?php $doctors_limit = empty($v['sg_doctors_limit'])?'no limit':$v['sg_doctors_limit'];?>
                <?php $explain = 'doctors：'.$doctors_limit.' templates：'.$v['sg_template_number'].' Charge standard:'.$v['sg_price'].' Additional features:'.$v['function_str'];?>
          ...
          <tr>
            ...
            <td><select name="sc_id" id="sc_id">
                <option value="0">please choose</option>
                <?php if(!empty($output['clic_class']) && is_array($output['clic_class'])){ ?>
                <?php foreach($output['clic_class'] as $k => $v){ ?>
          ...
          <tr>
            ...
            <td><a href="###" id="btn_select_category" class="btn">+Select add category</a>
              ...
              <input id="clic_class" name="clic_class" type="hidden" />
              <span></span>
          </td>
          </tr>
          <tr>
            <td colspan="2"><table bappointment="0" cellpadding="0" cellspacing="0" id="table_category" class="type">
                <thead>
                  <tr>
    ...
    <div class="bottom"><a id="btn_apply_clic_next" href="javascript:;" class="btn">Next</a></div>
  </div>
</div>
