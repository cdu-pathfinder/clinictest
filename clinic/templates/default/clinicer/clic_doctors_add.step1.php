...
<div class="wrapper_search">
  <div class="wp_sort">
    <div id="dataLoading" class="wp_data_loading">
      <div class="data_loading"><?php echo $lang['clic_doctors_step1_loading'];?></div>
    </div>
    <div class="sort_selector">
      <div class="sort_title"><?php echo $lang['clic_doctors_step1_choose_common_category'];?>
        <div class="text" id="commSelect">
            <div><?php echo $lang['clic_doctors_step1_please_select'];?></div>
            <div class="select_list" id="commListArea">
              <ul>
                ...
                <?php }?>
                <?php }?>
                <li id="select_list_no" <?php if (!empty($output['staple_array'])) {?>style="display: none;"<?php }?>><span class="title"><?php echo $lang['clic_doctors_step1_no_common_category'];?></span></li>
              </ul>
            </div>
        </div>
      ...
      <div class="sort_list">
        <div class="wp_category_list">
          <div id="class_div_1" class="category_list">
            <ul>
              <?php if(isset($output['doctors_class']) && !empty($output['doctors_class']) ) {?>
  ...
  <div class="alert">
    <dl class="hover_tips_cont">
      <dt id="commodityspan"><span style="color:#F00;"><?php echo $lang['clic_doctors_step1_please_choose_category'];?></span></dt>
      <dt id="commoditydt" style="display: none;" class="current_sort"><?php echo $lang['clic_doctors_step1_current_choose_category'];?><?php echo $lang['nc_colon'];?></dt>
  ...
  <div class="wp_confirm">
    <form method="get">
      ...
      <input type="hidden" name="op" value="edit_doctors" />
      <input type="hidden" name="commonid" value="<?php echo $output['commonid'];?>" />
      <input type="hidden" name="ref_url" value="<?php echo $_GET['ref_url'];?>" />
      <?php } else {?>
      <input type="hidden" name="act" value="clic_doctors_add" />
      <input type="hidden" name="op" value="add_step_two" />
      <?php }?>
      <input type="hidden" name="class_id" id="class_id" value="" />
      <input type="hidden" name="t_id" id="t_id" value="" />
      <div class="bottom tc">
      <label class="submit-bappointment"><input disabled="disabled" nctype="buttonNextStep" value="<?php echo $lang['clic_doctors_add_next'];?>，填写商品信息" type="submit" class="submit"style=" width: 200px;" /></label>
      </div>
    </form>
  </div>
</div>
...
<script src="<?php echo clinic_RESOURCE_SITE_URL;?>/js/clic_doctors_add.step1.js"></script> 
<script>
SEARCHKEY = '<?php echo $lang['clic_doctors_step1_search_input_text'];?>';
