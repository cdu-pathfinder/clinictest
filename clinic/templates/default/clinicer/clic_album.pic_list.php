...
<div id="pictureFolder" class="ncsc-picture-folder">
  <dl class="ncsc-album-intro">
    ...
    <dd class="album-covers">
      ...
      <img id="aclass_cover" src="<?php echo cthumb($output['class_info']['aclass_cover'], 60,$_SESSION['clic_id']);?>">
      <?php }else{?>
      ...
      <tr>
        ...
        <td><?php if(is_array($output['pic_list']) && !empty($output['pic_list'])){?>
          ...
          <dd id="batchClass" style=" display:none;">
            ...
            <?php }else{?>
            <span><?php echo $lang['album_plist_move_album_only_one'];?><a href="JavaScript:void(0);" uri="index.php?act=clic_album&op=album_add" nc_type="dialog" dialog_title="<?php echo $lang['album_class_add'];?>"><?php echo $lang['album_class_add'];?></a><?php echo $lang['album_plist_move_album_only_two'];?></span>
            <?php }?>
          </dd>
          </dl>
          <?php }?></td>
        ...
        <td class="w100"><?php if(is_array($output['pic_list']) && !empty($output['pic_list'])){?>
          <form name="select_sort" id="select_sort">
            <input type="hidden" name="act" value="clic_album" />
    ...
    <div class="ncsc-picture-list">
      ...
      <ul>
        ...
        <li>
          <dl>
            <dt>
              <div class="picture"><a href="index.php?act=clic_album&op=album_pic_info&id=<?php echo $v['apic_id'];?>&class_id=<?php echo $v['aclass_id']?><?php if(!empty($_GET['sort'])){?>&sort=<?php echo $_GET['sort']; }?>&curpage=<?php echo ceil((12*($curpage-1)+$ii)/7);?>&start_index=<?php echo ($tmp = (12*($curpage-1)+$ii)%7) ? $tmp-1 :6;?>"> <img id="img_<?php echo $v['apic_id'];?>" src="<?php echo thumb($v, 240);?>"></a></div>
...
$(function() {
    ...
    $('input[nctype="replace_image"]').each(function(){
        $(this).fileupload({
            dataType: 'json',
            url: '<?php echo clinic_SITE_URL;?>/index.php?act=clic_album&op=replace_image_upload&id=' + $(this).attr('id'),
    ...
    $('#fileupload').fileupload({
        dataType: 'json',
        url: '<?php echo clinic_SITE_URL;?>/index.php?act=clic_album&op=image_upload',
        add: function (e,data) {
...
function _save(obj){
		$.post('index.php?act=clic_album&op=change_pic_name', {id:obj.id,name:obj.value}, function(data) {
			if(data == 'false'){
...
function submit_form(type){
	...
	$('#checkboxform').attr('action','<?php echo clinic_SITE_URL?>/index.php?act=clic_album&op=album_pic_'+type);
	ajaxpost('checkboxform', '', '', 'onerror');
}
// 相册封面
function cover(id){
	if($('#aclass_cover').attr('src') != $('#img_'+id).attr('src')){
		ajaxget('index.php?act=clic_album&op=change_album_cover&id='+id);
	}else{
