...
<div class="tabmenu">
    <ul id="listpj" class="tab">
        <li class="active"><a href="<?php echo urlclinic('clic_evaluate', 'list');?>"><?php echo $lang['member_evaluation_frombuyer'];?></a></li>
    </ul>
</div>
<form method="get">
  <table class="search-form">
    <input type="hidden" name="act" value="clic_evaluate" />
    <input type="hidden" name="op" value="list" />
    <tr>
      <td>&nbsp;</td>
      <th class="w110">商品名称</th>
      <td class="w160"><input type="text" class="text w150" name="doctors_name" value="<?php echo $_GET['doctors_name'];?>"/></td>
...
<table class="ncsc-table-style">
    ...
    <tbody>
        <?php if (is_array($output['doctorsevallist']) && !empty($output['doctorsevallist'])) { ?>
        <?php foreach ((array)$output['doctorsevallist'] as $k=>$v){?>
        <tr>
            <th></th>
            <th>
                <span class="doctors-name"><a target="_blank" href="<?php echo urlclinic('doctors', 'index', array('doctors_id' => $v['geval_doctorsid']));?>"><?php echo $v['geval_doctorsname']?></a></span>
...
$(document).ready(function(){
    ...
    $('#btn_explain_submit').on('click', function() {
        ...
        $.post("<?php echo urlclinic('clic_evaluate', 'explain_save');?>",{
            geval_id: geval_id,
