...
<form method="get" action="index.php">
  <input type="hidden" name="act" value="clic_return" />
  <input type="hidden" name="lock" value="<?php echo $_GET['lock']; ?>" />
  <table class="search-form">
    <tr>
      <td>&nbsp;</td><th><?php echo $lang['return_appointment_add_time'];?></th>
      ...
      <td class="w80"><select name="state">
        ...
        <th class="w120"><select name="type">
          <option value="appointment_sn" <?php if($_GET['type'] == 'appointment_sn'){?>selected<?php }?>><?php echo $lang['return_appointment_appointmentsn']; ?></option>
          <option value="return_sn" <?php if($_GET['type'] == 'return_sn'){?>selected<?php }?>><?php echo $lang['return_appointment_returnsn']; ?></option>
          <option value="buyer_name" <?php if($_GET['type'] == 'buyer_name'){?>selected<?php }?>><?php echo $lang['return_appointment_buyer']; ?></option>
        </select></th>
...
<table class="ncsc-table-style">
  <thead>
    <tr>
      ...
      <th class="w70"><?php echo $lang['return_appointment_return'];?></th>
  ...
  <tbody>
    ...
    <tr class="bd-line" >
        <td></td>
        <td class="w50"><div class="pic-thumb"><a href="<?php echo urlclinic('doctors','index',array('doctors_id'=> $val['doctors_id']));?>" target="_blank"><img src="<?php echo thumb($val,60);?>"/></a></div></td>
        <td class="tl" title="<?php echo $val['clic_name']; ?>">
		<dl class="doctors-name"><dt><a href="<?php echo urlclinic('doctors','index',array('doctors_id'=> $val['doctors_id']));?>" target="_blank"><?php echo $val['doctors_name']; ?></a></dt>
        ...
        <dd><?php echo $lang['return_appointment_returnsn'].$lang['nc_colon'];?><?php echo $val['refund_sn']; ?></dd></dl></td>
        <td><?php echo $lang['currency'];?><?php echo $val['refund_amount'];?></td>
      <td><?php echo $val['return_type'] == 2 ? $val['doctors_num']:'无';?></td>
      ...
      <td class="nscs-table-handle">
        ...
        <span><a href="javascript:void(0)" class="btn-blue" nc_type="dialog" dialog_title="处理" dialog_id="return_appointment" dialog_width="480"
            uri="index.php?act=clic_return&op=edit&return_id=<?php echo $val['refund_id']; ?>"><i class="icon-edit"></i><p>处理</p></a></span>
        <?php } else { ?>
       <span> <a href="javascript:void(0)" class="btn-orange" nc_type="dialog" dialog_title="<?php echo $lang['nc_view'];?>" dialog_id="return_appointment" dialog_width="480" uri="index.php?act=clic_return&op=view&return_id=<?php echo $val['refund_id']; ?>"><i class="icon-eye-open"></i><p><?php echo $lang['nc_view'];?></p></a></span>
        <?php } ?>
        <?php if ($val['seller_state'] == 2 && $val['return_type'] == 2 && $val['doctors_state'] == 2) { ?>
        <span><a href="javascript:void(0)" class="btn-green" nc_type="dialog" dialog_title="<?php echo $lang['nc_edit'];?>" dialog_id="return_appointment" dialog_width="480" uri="index.php?act=clic_return&op=receive&return_id=<?php echo $val['refund_id']; ?>"><i class="icon-check-sign"></i><p><?php echo '收货';?></p></a></span>
        <?php } ?>
        </td>
    </tr>
    <?php } ?>
