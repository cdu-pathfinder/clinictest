...
<div id="pictureFolder" class="ncsc-picture-folder">
  <dl class="ncsc-album-intro">
    <dt class="albume-name"><a href="index.php?act=clic_album&op=album_pic_list&id=<?php echo $output['class_info']['aclass_id']?>"><?php echo $output['class_info']['aclass_name']?></a></dt>
    <dd class="album-covers">
      <?php if($output['class_info']['aclass_cover'] != ''){ ?>
      <img src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_doctorS.DS.$_SESSION['clic_id'].DS.str_replace('_small', '_tiny', $output['class_info']['aclass_cover']);?>">
      <?php }else{?>
...
<script>
  $(function() {
    ...
    var galleries = $('.ad-gallery').adGallery({loader_image:'<?php echo clinic_TEMPLATES_URL;?>/images/loading.gif', start_at_index:<?php echo intval($_GET['start_index']);?>, slideshow:{enable: false,start_label: '<?php echo $lang['album_pinfo_autoplay'];?>', stop_label: '<?php echo $lang['album_pinfo_suspend'];?>'}});
	...
	$('.ad-back').live('click',function(){
		...
		$('.ad-thumb-list').load('index.php?act=clic_album&op=album_ad_ajax&id=<?php echo $_GET['id'];?>&class_id=<?php echo $_GET['class_id'];?>&curpage='+curpage);
	});
...
function ajax_change_imgmessage(url){
	...
	$.getJSON("<?php echo clinic_SITE_URL; ?>/index.php?act=clic_album&op=ajax_change_imgmessage", {'url':url}, function(data){
