...
class chinabank{
    ...
    public function __construct($payment_info,$appointment_info){
    	$this->chinabank($payment_info,$appointment_info);
    }
    public function chinabank($payment_info = array(),$appointment_info = array()){
	...
	public function submit(){
		...
		$v_oid = $this->appointment['pay_sn'];															//订单号
		$v_amount = $this->appointment['pay_amount'];                  			//支付金额                 
        $v_moneytype = "CNY";                                           //币种
		$v_mid = $this->payment['payment_config']['chinabank_account'];	// 商户号，这里为测试商户号1001，替换为自己的商户号(老版商户号为4位或5位,新版为8位)即可
		$v_url = clinic_SITE_URL."/api/payment/chinabank/return_url.php";	// 请填写返回url,地址应为绝对路径,带有http协议
    ...
    $v_md5info = strtoupper(md5($text));                            //md5函数加密并转化成大写字母
		...
		'remark1'    	  => $this->appointment['appointment_type'],                          								// 备注字段1
		'remark2'    	  => ''                          									// 备注字段2
		);
