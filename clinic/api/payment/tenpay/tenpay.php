...
class tenpay{
    ...
    private $parameter;
    /**
     * 订单类型 doc_buy商品购买,predeposit预存款充值
     * @var unknown
     */
    ...
    public function __construct($payment_info,$appointment_info){
    	$this->tenpay($payment_info,$appointment_info);
    }
    public function tenpay($payment_info = array(),$appointment_info = array()){
    	if(!empty($payment_info) and !empty($appointment_info)){
	...
	public function get_payurl(){
		...
		/* 返回处理地址 */
		$return_url = clinic_SITE_URL."/api/payment/tenpay/return_url.php";
		...
		$sp_billno = $this->appointment['pay_sn'];
		
		/* 财付通交易单号，规则为：10位商户号+8位时间（YYYYmmdd)+10位流水号 */
		$transaction_id = $bargainor_id . $strDate . $strReq;
		
		/* 商品价格（包含运费），以分为单位 */
		$total_fee = floatval($this->appointment['pay_amount'])*100;
		...
		$reqHandler->setParameter("desc", $this->appointment['subject']);	        //商品名称
        $reqHandler->setParameter("attach", $this->appointment['appointment_type']);	//自定义参数doc_buy商品购买,predeposit预存款充值
	...
	public function return_verify(){
		...
		if($resHandler->isTenpaySign()) {
			...
			if( "0" == $pay_result ) {
				//判断返回金额
				$appointment_amount = $total_fee / 100;
				if (!empty($this->appointment['pdr_amount'])){
				    $this->appointment['pay_amount'] = $this->appointment['pdr_amount'];
				}
				if ($this->appointment['pay_amount'] != $appointment_amount){
					return false;
				}
				$this->appointment_type = $attach;
