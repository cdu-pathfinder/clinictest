...
class alipay{
    ...
    public function __construct($payment_info=array(),$appointment_info=array()){
    ...
    public function get_payurl(){
    	$this->parameter = array(
            ...
            'notify_url'	    => clinic_SITE_URL."/api/payment/alipay/notify_url.php",	//通知URL
            'sign_type'		    => 'MD5',				//签名方式
            'return_url'	    => clinic_SITE_URL."/api/payment/alipay/return_url.php",	//返回URL
            'extra_common_param'=> $this->appointment['appointment_type'],//doc_buy商品购买,predeposit预存款充值
            'subject'		    => $this->appointment['subject'],	//商品名称
            'body'			    => $this->appointment['pay_sn'],	//商品描述
            'out_trade_no'	    => $this->appointment['pay_sn'],		//外部交易编号
            'payment_type'	    => 1,							//支付类型
            'logistics_type'    => 'EXPRESS',					//物流配送方式：POST(平邮)、EMS(EMS)、EXPRESS(其他快递)
            'logistics_payment'	=> 'BUYER_PAY',				     //物流费用付款方式：clinicer_PAY(卖家支付)、BUYER_PAY(买家支付)、BUYER_PAY_AFTER_RECEIVE(货到付款)
            'receive_name'		=> $_SESSION['member_name'],//收货人姓名
            'receive_address'	=> 'N',	//收货人地址
            'receive_zip'		=> 'N',	//收货人邮编
            'receive_phone'		=> 'N',//收货人电话
            'receive_mobile'	=> 'N',//收货人手机
            'clinicer_email'		=> $this->payment['payment_config']['alipay_account'],	//卖家邮箱
            'price'             => $this->appointment['pay_amount'],//订单总价
            'quantity'          => 1,//商品数量
            'total_fee'         => 0,//物流配送费用
	...
	public function notify_verify() {
		...
		if (preg_match("/true$/i",$veryfy_result) && $mysign == $param["sign"])  {
		    $this->appointment_type = $param['extra_common_param'];
			return true;
		} else {
			return false;
		}
