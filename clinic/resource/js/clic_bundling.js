$(document).ready(function(){
	/* ajax添加商品  */
	$('#bundling_add_doctors').ajaxContent({
		event:'click', //mouseover
		loaderType:"img",
		loadingMsg:clinic_TEMPLATES_URL+"/images/loading.gif",
		target:'#bundling_add_doctors_ajaxContent'
	}).click(function(){
	    $(this).hide();
	    $('#bundling_add_doctors_delete').show();
	});
	
	$('#bundling_add_doctors_delete').click(function(){
	    $(this).hide();
	    $('#bundling_add_doctors_ajaxContent').html('');
	    $('#bundling_add_doctors').show();
	});
	// 退拽效果
    $('tbody[nctype="bundling_data"]').sortable({ items: 'tr' });
    $('#doctors_images').sortable({ items: 'li' });
});
