    ...
    $.ajaxFileUpload({
        ...
        success : function (data, status) {
                    if (typeof(data.error) != 'undefined') {
                        alert(data.error);
                        $('img[nctype="' + id + '"]').attr('src',DEFAULT_doctorS_IMAGE);
                    } else {
                        $('input[nctype="' + id + '"]').val(data.name);
                        $('img[nctype="' + id + '"]').attr('src', data.thumb_name);
                        selectDefaultImage($('div[nctype="' + id + '"]'));      // 选择默认主图
                    }
                    $.getScript(clinic_RESOURCE_SITE_URL+ '/js/clic_doctors_add.step3.js');
                },
...
function selectDefaultImage($this) {
    ...
    $this.parents('li:first').find('a[nctype="del"]').click(function(){
        ...
        $this.prev().find('input').val('').end().find('img').attr('src', DEFAULT_doctorS_IMAGE);
    });
}
