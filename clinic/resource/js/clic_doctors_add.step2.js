$(function(){
    ...
    /* 商品图片ajax上传 */
    $('#doctors_image').fileupload({
        dataType: 'json',
        url: SITEURL + '/index.php?act=clic_doctors_add&op=image_upload&upload_type=uploadedfile',
        formData: {name:'doctors_image'},
        add: function (e,data) {
        	$('img[nctype="doctors_image"]').attr('src', clinic_TEMPLATES_URL + '/images/loading.gif');
            data.submit();
        },
            ...
            if (typeof(param.error) != 'undefined') {
                alert(param.error);
                $('img[nctype="doctors_image"]').attr('src',DEFAULT_doctorS_IMAGE);
            } else {
                $('input[nctype="doctors_image"]').val(param.name);
                $('img[nctype="doctors_image"]').attr('src',param.thumb_name);
            }
        }
    });
    ...
    $('a[nctype="show_image"]').unbind().ajaxContent({
        ...
        $(this).hide();
        $('a[nctype="del_doctors_demo"]').show();
    });
    $('a[nctype="del_doctors_demo"]').unbind().click(function(){
        $('#demo').html('');
        $(this).hide();
    ...
    $('#add_album').fileupload({
        dataType: 'json',
        url: SITEURL+'/index.php?act=clic_doctors_add&op=image_upload',
...
function insert_img(name, src) {
    $('input[nctype="doctors_image"]').val(name);
    $('img[nctype="doctors_image"]').attr('src',src);
}
