$(function(){
    ...
    $(".item").hover(
        function() {
            $(this).find(".doctors-info").animate({"top": "180px"}, 400, "swing");
        },function() {
            $(this).find(".doctors-info").stop(true,false).animate({"top": "230px"}, 400, "swing");
        }
    );
    // 加入购物车
    $('a[nctype="add_cart"]').click(function() {
        ...
        addcart(data_str.doctors_id);
    });
    // 立即购买
    $('a[nctype="buy_now"]').click(function(){
        eval('var data_str = ' + $(this).attr('data-param'));
        $("#doctors_id").val(data_str.doctors_id+'|1');
        $("#buynow_form").submit();
    })
    // 图片切换效果
    $('.doctors-pic-scroll-show').find('a').mouseover(function(){
        $(this).parents('li:first').addClass('selected').siblings().removeClass('selected');
        var _src = $(this).find('img').attr('src');
        _src = _src.replace('_60.', '_240.');
        $(this).parents('.doctors-content').find('.doctors-pic').find('img').attr('src', _src);
    });
});
...
/* 加入购物车 */
function addcart(doctors_id) {
    var url = 'index.php?act=cart&op=add';
    $.getJSON(url, {'doctors_id':doctors_id, 'quantity':1}, function(data) {
        if (data != null) {
            if (data.state) {
                // 头部加载购物车信息
