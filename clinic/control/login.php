...
class loginControl extends BaseHomeControl {
	...
	public function indexOp(){
		...
		if ($result !== false){
			...
			if (processClass::islock('login')) {
				showDialog($lang['nc_common_op_repeat'],clinic_SITE_URL);
			}
	...
	private function mergecart($member_info = array()){
        ...
        $mode_doctors= Model('doctors');
        $condition = array();
        if (!empty($_SESSION['clic_id'])) {
            $condition['clic_id'] = array('neq',$_SESSION['clic_id']);
        }
        $condition['doctors_id'] = array('in',array_keys($cart_new_list));
		$doctors_list = Model('doctors')->getdoctorsOnlineList($condition);
		if (!empty($doctors_list)){
			foreach ($doctors_list as $doctors_info){
			    $doctors_info['buyer_id']	= $member_info['member_id'];
			    $model_cart->addCart($doctors_info,'db',$cart_new_list[$doctors_info['doctors_id']]['doctors_num']);
			}
		}
		//最后清空登录前购物车内容
	...
	public function find_passwordOp(){
		...
		if($result){
			...
			showDialog($lang['login_password_email_success'],clinic_SITE_URL,'succ');
		}else{
