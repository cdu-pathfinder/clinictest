...
class clic_specControl extends BaseclinicerControl {
    ...
    public function indexOp() {
        ...
        $gc_list = $model_doctorsclass->getdoctorsClass($_SESSION['clic_id']);
        Tpl::output('gc_list', $gc_list);
        
        $this->_profile_menu('spec', 'spec');
        Tpl::output('menu_sign', 'clic_spec');
        Tpl::showpage('clic_spec.index');
    }
    ...
    public function add_specOp(){
        ...
        // 分类信息
        $gc_info = Model('doctors_class')->getdoctorsClassInfo(array('gc_id' => $gc_id), 'gc_id,gc_name');
        ...
        // 规格值信息
        $sp_value_list = $model_spec->getSpecValueList(array('clic_id' => $_SESSION['clic_id'], 'sp_id' => $sp_id, 'gc_id'=>$gc_id));
        Tpl::output('sp_value_list', $sp_value_list);
        
        $this->_profile_menu('spec_add', 'add');
        Tpl::output('menu_sign', 'clic_spec');
        Tpl::showpage('clic_spec.add', 'null_layout');
    }
    ...
    public function ajax_delspecOp(){
        ...
        $rs = Model('spec')->delSpecValue(array('sp_value_id' => $sp_value_id, 'clic_id' => $_SESSION['clic_id']));
        if ($rs) {
            echo 'true'; exit();
        } else {
            echo 'false'; exit();
    ...
    public function ajax_classOp() {
        ...
        // 验证分类是否存在
        $gc_info = $model_doctorsclass->getdoctorsClassInfo(array('gc_id' => $id), 'type_id');
        if (empty($gc_info)) {
            echo 'false'; exit();
        }
        ...
        // 读取商品分类
        if ($deep != 4) {
            $gc_list = $model_doctorsclass->getdoctorsClass($_SESSION['clic_id'], $id, $deep);
        }
        // 分类不为空输出分类信息
        if (!empty($gc_list)) {
    ...
    private function _profile_menu($menu_type, $menu_key) {
        ...
        switch ($menu_type) {
            case 'spec':
                $menu_array = array(
                    array('menu_key' => 'spec', 'menu_name' => L('nc_member_path_clic_spec'), 'menu_url' => 'index.php?act=clic_spec')
                );
            break;
        }
