...
class statistics_saleControl extends BaseclinicerControl {
	...
	public function sale_statisticsOp() {
		if($_GET['type'] == 'month'){
			...
			$model = Model();
			$flow_array = $model->table('salenum')->field('date,sum(salenum) as sum')->where(array('date'=>array('in',$request_date_str),'clic_id'=>$_SESSION['clic_id']))->group('date')->select();
			//整理流量信息数组
			$result_clicknum_str = '';
			...
			$request_date_str = '';
				...
				$to = $_GET['add_time_to'];
				$main_title = Language::get('clic_sale_search_result');
	 ...
	 * @return 
	 */
	public function doctors_sale_statisticsOp() {
		if($_GET['type'] == 'month'){
			$year  = date('Y',time());
			$month = date('m',time());
			$day31 = array('01','03','05','07','08','10','12');
			...
			if($_GET['add_time_from'] != '' && $_GET['add_time_to'] != ''){
				...
				$from = $_GET['add_time_from'];
				$to = $_GET['add_time_to'];
				$main_title = Language::get('clic_sale_rank_search_result');
				$sub_title  = substr($from,0,4).'.'.substr($from,4,2).'.'.substr($from,6,2).'-'.substr($to,0,4).'.'.substr($to,4,2).'.'.substr($to,6,2);
			}else{
				//默认显示本周商品流量排名
				$day = date('l',time());
		...
		$field = 'sum('.$flow_tablename.'.salenum) as sum,'.$flow_tablename.'.doctors_id,doctors.doctors_name';
		$where = $flow_tablename.".date in (".$request_date_str.") and ".$flow_tablename.".clic_id = '".$_SESSION['clic_id']."'";
		...
		Tpl::output('main_title',$main_title);
		Tpl::output('sub_title',$sub_title);
		self::profile_menu('doctors_sale');
		Tpl::output('menu_sign','sale_statistics');
		Tpl::output('menu_sign_url','index.php?act=statistics&op=sale_statistics');
		Tpl::output('menu_sign1','doctors_sale');
		Tpl::showpage('doctors_sale');
	}
	...
	private function profile_menu($menu_key='') {
        $menu_array	= array(
            1=>array('menu_key'=>'clic_sale','menu_name'=>Language::get('stat_clic_sale'),	'menu_url'=>'index.php?act=statistics_sale&op=sale_statistics'),
            2=>array('menu_key'=>'doctors_sale','menu_name'=>Language::get('stat_doctors_sale'),	'menu_url'=>'index.php?act=statistics_sale&op=doctors_sale_statistics')
        );
