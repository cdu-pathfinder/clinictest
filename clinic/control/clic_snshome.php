...
class clic_snshomeControl extends BaseclicSnsControl{
	...
	public function indexOp(){
		//获得店铺ID
		$sid	= intval($_GET['sid']);
		$this->getclicInfo($sid);
		
		// where 条件
		$where = array();
		$where['strace_state'] = 1;
		$where['strace_clicid'] = $sid;
		if($_GET['type'] != ''){
		...
		$strace_array = $model_stracelog->getclicSnsTracelogList($where, '*', 'strace_id desc', 0, 40);
		// 整理 
	...
	public function commenttopOp(){
		...
		if($stid > 0){
			...
			$countnum = $model_clicsnscomment->getclicSnsCommentCount($where);
			
			//动态列表
			$commentlist = $model_clicsnscomment->getclicSnsCommentList($where, '*', 'scomm_id desc', 10);
			
			// 更新评论数量
			Model('clic_sns_tracelog')->editclicSnsTracelog(array('strace_comment'=>$countnum), array('strace_id'=>$stid));
		}
		...
		Tpl::showpage('clic_snscommentlist','null_layout');
	}
	/**
	 * 评论列表
	 */
	public function commentlistOp(){
		...
		if($stid > 0){
			...
			//评价列表
			$commentlist = $model_clicsnscomment->getclicSnsCommentList($where, '*', 'scomm_id desc', 0, 10);
			
			// 更新评论数量
			$commentlist = Model('clic_sns_tracelog')->editclicSnsTracelog(array('strace_comment'=>$countnum), array('strace_id'=>$stid));
		}
...
// 		//查询会员信息
		...
		$result = Model('clic_sns_comment')->saveclicSnsComment($insert_arr);
		if ($result){
			// 原帖增加评论次数
			$where = array('strace_id'=>$stid);
			$update = array('strace_comment'=>array('exp','strace_comment+1'));
			$rs = Model('clic_sns_tracelog')->editclicSnsTracelog($update, $where);
			//建立cookie
			...
			if ($_POST['showtype'] == 1){
				$js .="$('#tracereply_".$stid."').load('index.php?act=clic_snshome&op=commenttop&id=".$stid."');";
			}else {
				$js .="$('#tracereply_".$stid."').load('index.php?act=clic_snshome&op=commentlist&id=".$stid."');";
			}
	...
	public function addforwardOp(){
		...
		//查询原帖信息
		$model_stracelog = Model('clic_sns_tracelog');
		$stracelog_info = $model_stracelog->getclicSnsTracelogInfo(array('strace_id' => $stid));
		if (empty($stracelog_info)){
			showDialog(Language::get('sns_forward_fail'),'','error');
		}
		if($stracelog_info['strace_content'] == ''){
			$data = json_decode($stracelog_info['strace_doctorsdata'],true);
			if( CHARSET == 'GBK') {
		...
		$insert_arr['trace_content']			= "<dl class=\"fd-wrap\">
														<dt>
															<h3><a href=\"index.php?act=clic_snshome&sid=".$stracelog_info['strace_clicid']."\" target=\"_blank\">".$stracelog_info['strace_clicname']."</a>".Language::get('nc_colon')."
		...
		if ($result){
			...
			$model->editclicSnsTracelog($update, $where);
	...
	public function deltraceOp(){
		...
		//删除动态
		$result = Model('clic_sns_tracelog')->delclicSnsTracelog(array('strace_id'=>$stid , 'strace_clicid'=>$_SESSION['clic_id'] ));
		if ($result){
			//删除对应的评论
			Model('clic_sns_comment')->delclicSnsComment(array('strace_id' => $stid));
	...
	public function delcommentOp(){
		...
		$scomment_info	= $model_clicsnscomment->getclicSnsCommentInfo($where);
		if(empty($scomment_info)){
			showDialog(L('wrong_argument'),'','error');
		}
		
		// 删除评论
		$result = $model_clicsnscomment->delclicSnsComment($where);
		if ($result){
			// 更新动态统计信息
			$where = array('strace_id'=>$scomment_info['strace_id']);
			$update = array('strace_comment'=>array('exp','strace_comment-1'));
			Model('clic_sns_tracelog')->editclicSnsTracelog($update, $where);
	...
	public function straceinfoOp(){
		...
		$model_stracelog = Model('clic_sns_tracelog');
		$strace_info = $model_stracelog->getclicSnsTracelogInfo(array('strace_id' => $stid));
		if(!empty($strace_info)){
			if($strace_info['strace_content'] == ''){
				$content = $model_stracelog->spellingStyle($strace_info['strace_type'], json_decode($strace_info['strace_doctorsdata'],true));
				$strace_info['strace_content'] = str_replace("%siteurl%", clinic_SITE_URL.DS, $content);
			}
		}
		Tpl::output('strace_info', $strace_info);
		//验证码
		Tpl::output('nchash',substr(md5(clinic_SITE_URL.$_GET['act'].$_GET['op']),0,8));
		Tpl::showpage('clic_snstraceinfo');
	}
	/**
	 * 验证用户是否登录
	 */
