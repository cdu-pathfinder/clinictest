...
class clic_consultControl extends BaseclinicerControl {
	public function __construct() {
		parent::__construct();
		Language::read('member_clic_consult_index');
	}
	...
	public function consult_listOp(){
		...
		$search_array['clic_id']	= "{$_SESSION['clic_id']}";
		...
		Tpl::output('menu_sign_url','index.php?act=clic_consult&op=consult_list');
		Tpl::output('menu_sign1',$_GET['type']);
		Tpl::showpage('clic_consult_manage');
	}
	...
	public function drop_consultOp(){
		...
		$state	= $consult->dropConsult($consult_id,$_SESSION['clic_id']);
		if($state) {
			showDialog(Language::get('clic_consult_drop_success'),'reload','succ');
		} else {
			showDialog(Language::get('clic_consult_drop_fail'));
		}
	}
	...
	public function reply_consultOp(){
		...
		Tpl::showpage('clic_consult_form','null_layout');
	}
	...
	public function reply_saveOp(){
		...
		$condtion_arr = array();
		$condtion_arr['clic_id'] = "{$_SESSION['clic_id']}";
	...
	private function profile_menu($menu_type,$menu_key='',$array=array()) {		
		...
		switch ($menu_type) {
			case 'consult':
				$menu_array	= array(
				1=>array('menu_key'=>'consult_list',	'menu_name'=>Language::get('nc_member_path_all_consult'),			'menu_url'=>'index.php?act=clic_consult&op=consult_list'),
				2=>array('menu_key'=>'to_reply',	'menu_name'=>Language::get('nc_member_path_unreplied_consult'),			'menu_url'=>'index.php?act=clic_consult&op=consult_list&type=to_reply'),
				3=>array('menu_key'=>'replied',	'menu_name'=>Language::get('nc_member_path_replied_consult'),			'menu_url'=>'index.php?act=clic_consult&op=consult_list&type=replied'));
				break;
		}
		if(!empty($array)) {
			$menu_array[] = $array;
