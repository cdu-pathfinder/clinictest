...
class clic_complainControl extends BaseclinicerControl{
    //定义投诉状态常量
    const STATE_NEW = 10;
    ...
    public function complain_showOp() {
        ...
        $appointment_info = $model_appointment->getappointmentInfo(array('appointment_id' => $complain_info['appointment_id']));
        if($appointment_info['clic_id'] != $_SESSION['clic_id']) {
    ...
    public function appeal_saveOp() {
        ...
        $this->recordclinicerLog('投诉申诉处理，投诉编号：'.$complain_id);
        showDialog(Language::get('appeal_submit_success'),'index.php?act=clic_complain','succ');
    }
    ...
    public function apply_handleOp() {
        ...
        $this->recordclinicerLog('投诉申请仲裁，投诉编号：'.$complain_id);
        showMessage(Language::get('handle_submit_success'),'index.php?act=clic_complain');
    }
    ...
    private function get_complain_info($complain_id) {
        ...
        if($complain_info['accused_id'] != $_SESSION['clic_id']) {
            showMessage(Language::get('para_error'),'','html','error');
        }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            array('menu_key'=>'complain_accused_list','menu_name'=>Language::get('complain_manage_title'),'menu_url'=>'index.php?act=clic_complain')
        );
