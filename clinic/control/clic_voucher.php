...
class clic_voucherControl extends BaseclinicerControl{
	//定义代金券类常量
	...
	public function __construct() {
		...
		if (C('voucher_allow') != 1){
			showMessage(Language::get('voucher_unavailable'),'index.php?act=clic','html','error');
		}
		//申请记录状态
	...
	public function templatelistOp(){
        ...
        //查询是否存在可用套餐
        $model = Model('voucher');
        $current_quota = $model->getCurrentQuota($_SESSION['clic_id']);
        Tpl::output('current_quota',$current_quota);
		//查询列表
		$param = array();
		$param['voucher_t_clic_id'] = $_SESSION['clic_id'];
		...
		$list = $model->table('voucher_template')->where($param)->appointment('voucher_t_id desc')->page(10)->select();
		if(is_array($list)){
			foreach ($list as $key=>$val){
				if (!$val['voucher_t_customimg'] || !file_exists(BASE_UPLOAD_PATH.DS.ATTACH_VOUCHER.DS.$_SESSION['clic_id'].DS.$val['voucher_t_customimg'])){
					$list[$key]['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.defaultdoctorsImage(60);
				}else{
					$list[$key]['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.ATTACH_VOUCHER.DS.$_SESSION['clic_id'].DS.str_ireplace('.', '_small.', $val['voucher_t_customimg']);
				}
			}
		}
		...
		Tpl::showpage('clic_voucher_template.index') ;
	}
	...
	public function quotaaddOp(){
		if (chksubmit()){
            ...
            //获取该用户已有套餐
            $current_quota = $model_voucher->getCurrentQuota($_SESSION['clic_id']);
            ...
            if(empty($current_quota)) {
                ...
                $param['quota_clicid'] = $_SESSION['clic_id'];
                $param['quota_clicname'] = $_SESSION['clic_name'];
            ...
            //记录店铺费用
            $this->recordclicCost($current_price * $quota_quantity, '购买代金券套餐');

            $this->recordclinicerLog('购买'.$quota_quantity.'份代金券套餐，单价'.$current_price.L('nc_yuan'));

            if($reault){
                showDialog(Language::get('voucher_apply_buy_succ'),'index.php?act=clic_voucher&op=quotalist','succ');
            } else {
                showDialog(Language::get('nc_common_op_fail'),'index.php?act=clic_voucher&op=quotalist');
            }
        }else {
            //输出导航
	        self::profile_menu('quota_add','quotaadd');
	        Tpl::output('menu_sign','clic_voucher');
			Tpl::output('menu_sign_url','index.php?act=clic_voucher');
	        Tpl::showpage('clic_voucher_quota.add');
		}
    }
	/*
	 * 代金券模版添加 
    ...
    public function templateaddOp(){
        $model = Model('voucher');
        //查询当前可以套餐
        $quotainfo = $model->getCurrentQuota($_SESSION['clic_id']);
        if(empty($quotainfo)){
        	showMessage(Language::get('voucher_template_quotanull'),'index.php?act=clic_voucher&op=quotaadd','html','error');
        }
        //查询该套餐下代金券模板列表
        ...
        if ($count >= C('promotion_voucher_clictimes_limit')){
        	$message = sprintf(Language::get('voucher_template_noresidual'),C('promotion_voucher_clictimes_limit'));
        	showMessage($message,'index.php?act=clic_voucher&op=templatelist','html','error');
        }

        //查询面额列表
        $pricelist =  $model->table('voucher_price')->appointment('voucher_price asc')->select();
        if(empty($pricelist)){
        	showMessage(Language::get('voucher_template_pricelisterror'),'index.php?act=clic_voucher&op=templatelist','html','error');
        }
        if(chksubmit()){
	        //验证提交的内容面额不能大于限额 
	        ...
	        if ($error){
	        	...
	        	$insert_arr['voucher_t_clic_id'] = $_SESSION['clic_id'];
	        	$insert_arr['voucher_t_clicname'] = $_SESSION['clic_name'];
		        ...
		        if (!empty($_FILES['customimg']['name'])){
		        	...
		        	$upload->set('default_dir',	ATTACH_VOUCHER.DS.$_SESSION['clic_id']);
	            ...
	            if($rs){
	                showDialog(Language::get('nc_common_save_succ'),'index.php?act=clic_voucher&op=templatelist','succ'); 
	            }else{
	                showDialog(Language::get('nc_common_save_fail'),'index.php?act=clic_voucher&op=templatelist','error'); 
	            }
	        }
        }else{
	        TPL::output('type','add');
	        ...
	        Tpl::output('menu_sign','clic_voucher');
			Tpl::output('menu_sign_url','index.php?act=clic_voucher');
	        Tpl::showpage('clic_voucher_template.add');
        }        
    }
	/*
	 * 代金券模版编辑
    ...
    public function templateeditOp(){
    	...
    	if ($t_id <= 0){
    		showMessage(Language::get('wrong_argument'),'index.php?act=clic_voucher&op=templatelist','html','error');
    	}
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        //查询套餐信息
        $quotainfo = $model->table('voucher_quota')->where(array('quota_id'=>$t_info['voucher_t_quotaid'],'quota_clicid'=>$_SESSION['clic_id']))->find();
        if(empty($quotainfo)){
        	showMessage(Language::get('voucher_template_quotanull'),'index.php?act=clic_voucher&op=quotaadd','html','error');
        }
        //查询面额列表
        ...
        if(chksubmit()){
	        ...
	        if ($error){
		        ...
		        if (!empty($_FILES['customimg']['name'])){
					...
					if ($result){
						...
						if (!empty($t_info['voucher_t_customimg'])){//如果模板存在，则删除原模板图片
							@unlink(BASE_UPLOAD_PATH.DS.ATTACH_VOUCHER.DS.$_SESSION['clic_id'].DS.$t_info['voucher_t_customimg']);
							@unlink(BASE_UPLOAD_PATH.DS.ATTACH_VOUCHER.DS.$_SESSION['clic_id'].DS.str_ireplace('.', '_small.', $t_info['voucher_t_customimg']));
						}
	            ...
	            if($rs){
	                showDialog(Language::get('nc_common_op_succ'),'index.php?act=clic_voucher&op=templatelist','succ'); 
	            }else{
	                showDialog(Language::get('nc_common_op_fail'),'index.php?act=clic_voucher&op=templatelist','error'); 
	            }
	        }
        }else{
	        if (!$t_info['voucher_t_customimg'] || !file_exists(BASE_UPLOAD_PATH.DS.ATTACH_VOUCHER.DS.$_SESSION['clic_id'].DS.$t_info['voucher_t_customimg'])){
	        	$t_info['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.defaultdoctorsImage(240);
	        }else{
	        	$t_info['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.ATTACH_VOUCHER.DS.$_SESSION['clic_id'].DS.str_ireplace('.', '_small.', $t_info['voucher_t_customimg']);
	        }
    ...
    public function templateinfoOp(){
        ...
        Tpl::output('menu_sign','clic_voucher');
        Tpl::output('menu_sign_url','index.php?act=clic_voucher');
        Tpl::showpage('clic_voucher_template.info');
    }
	/*
	 * 把代金券模版设为失效
	 */
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'voucher':
				$menu_array = array(
				1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_clic_voucher'), 'menu_url'=>'index.php?act=clic_voucher&op=templatelist'),
				);
				break;
			case 'quota_add':
				$menu_array = array(
				1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_clic_voucher'),	'menu_url'=>'index.php?act=clic_voucher&op=templatelist'),
				4=>array('menu_key'=>'quotaadd','menu_name'=>Language::get('voucher_applyadd'),	'menu_url'=>'index.php?act=clic_voucher&op=quotaadd')
				);
				break;
			case 'template':
				$menu_array = array(
				1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_clic_voucher'),	'menu_url'=>'index.php?act=clic_voucher&op=templatelist'),
				2=>array('menu_key'=>'templateadd','menu_name'=>Language::get('voucher_templateadd'),	'menu_url'=>'index.php?act=clic_voucher&op=templateadd'),
				);
				break;
			case 'templateedit':
				$menu_array = array(
