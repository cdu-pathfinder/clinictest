...
class clic_albumControl extends BaseclinicerControl {
	...
	public function __construct() {
		parent::__construct();
		Language::read('member_clic_album');
	}
	...
	public function album_cateOp(){
		 ...
		 * 验证是否存在默认相册
		 */
		$return = $model_album->checkAlbum(array('album_aclass.clic_id'=>$_SESSION['clic_id'],'is_default'=>'1'));
		if(!$return){
			$album_arr = array();
			$album_arr['aclass_name'] = Language::get('album_default_album');
			$album_arr['clic_id'] = $_SESSION['clic_id'];
		...
		$param['appointment']					= 'aclass_sort desc';
		if($_GET['sort'] != ''){
			switch ($_GET['sort']){
				case '0':
					$param['appointment']		= 'upload_time desc';
					break;
				case '1':
					$param['appointment']		= 'upload_time asc';
					break;
				case '2':
					$param['appointment']		= 'aclass_name desc';
					break;
				case '3':
					$param['appointment']		= 'aclass_name asc';
					break;
				case '4':
					$param['appointment']		= 'aclass_sort desc';
					break;
				case '5':
					$param['appointment']		= 'aclass_sort asc';
					break;
			}
		}
...
//		Tpl::output('show_page',$page->show());
		...
		Tpl::output('menu_sign_url','index.php?act=clic_album&op=album_cate');
		Tpl::output('menu_sign1','my_album');
		Tpl::showpage('clic_album.list');
	}
	/**
	 * 相册分类添加
	 *
	...
	public function album_addOp(){
		 ...
		 */
		$model_album = Model('album');
		$class_count = $model_album->countClass($_SESSION['clic_id']);
		Tpl::output('class_count',$class_count['count']);
		Tpl::showpage('clic_album.class_add','null_layout');
	}
	/**
	 * 相册保存
	 *
	...
	public function album_add_saveOp(){
		if (chksubmit()){
			...
			if($class_count['count'] >= 20){
				showDialog(Language::get('album_class_save_max_20'),'index.php?act=clic_album','error',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
			}
			/**
			 * 实例化相册模型
			 */
			...
			if($return){
				showDialog(Language::get('album_class_save_succeed'),'index.php?act=clic_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
			}
		}
	...
	public function album_editOp(){
		...
		$model_album = Model('album');
		$param = array();
		$param['field']		= array('aclass_id','clic_id');
		$param['value']		= array(intval($_GET['id']),$_SESSION['clic_id']);
		$class_info = $model_album->getOneClass($param);
		Tpl::output('class_info',$class_info);

		Tpl::showpage('clic_album.class_edit','null_layout');
	}
	/**
	 * 相册分类编辑保存
	 */
	public function album_edit_saveOp(){
		 ...
		 * 验证
		 */
		$return	= $model_album->checkAlbum(array('album_aclass.clic_id'=>$_SESSION['clic_id'],'album_aclass.aclass_id'=>intval($_POST['id'])));
		if($return){
			/**
			 * 更新
			 */
			...
			if($re){
				showDialog(Language::get('album_class_edit_succeed'),'index.php?act=clic_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
			}
		}else{
	...
	public function album_delOp(){
		 ...
		 * 验证
		 */
		$return = $model_album->checkAlbum(array('album_aclass.clic_id'=>$_SESSION['clic_id'],'album_aclass.aclass_id'=>intval($_GET['id']),'is_default'=>'0'));
		if(!$return){
		 ...
		 */
		$param = array();
		$param['field']		= array('is_default','clic_id');
		$param['value']		= array('1',$_SESSION['clic_id']);
		$class_info = $model_album->getOneClass($param);
		$param = array();
		$param['aclass_id'] = $class_info['aclass_id'];
		$return = $model_album->updatePic($param,array('aclass_id'=>intval($_GET['id'])));
		if($return){
			showDialog(Language::get('album_class_file_del_succeed'),'index.php?act=clic_album','succ');
		}else{
			showDialog(Language::get('album_class_file_del_lose'));
		}
	}
	...
	public function album_pic_listOp(){
		...
		$param['album_pic.clic_id']	= $_SESSION['clic_id'];
		if($_GET['sort'] != ''){
			switch ($_GET['sort']){
				case '0':
					$param['appointment']		= 'upload_time desc';
				...
				case '1':
					...
					break;
				case '2':
					$param['appointment']		= 'apic_size desc';
					break;
				case '3':
					$param['appointment']		= 'apic_size asc';
					break;
				case '4':
					$param['appointment']		= 'apic_name desc';
					break;
				case '5':
					$param['appointment']		= 'apic_name asc';
					break;
			}
		}
		...
		Tpl::showpage('clic_album.pic_list');
	}
	/**
	 * 图片列表，外部调用
	 */
	public function pic_listOp(){
		 ...
		 */
		$page	= new Page();
        if(in_array($_GET['item'] , array('doctors_image'))) {
            $page->setEachNum(12);
        } else {
            $page->setEachNum(14);
        }
		...
		if(!empty($_GET) && $_GET['id'] != '0'){
			 ...
			 */
			$cparam = array();
			$cparam['field']		= array('aclass_id','clic_id');
			$cparam['value']		= array(intval($_GET['id']),$_SESSION['clic_id']);
        ...
        switch($_GET['item']) {
        case 'doctors':
            Tpl::showpage('clic_doctors_add.step2_master_image','null_layout');
            break;
        case 'des':
            Tpl::showpage('clic_doctors_add.step2_desc_image','null_layout');
            break;
        case 'groupbuy':
            Tpl::showpage('clic_groupbuy.album','null_layout');
            break;
        case 'clic_sns_normal':
            Tpl::showpage('clic_sns_add.album', 'null_layout');
            break;
        case 'doctors_image':
            Tpl::output('color_id', $_GET['color_id']);
            Tpl::showpage('clic_doctors_add.step3_doctors_image', 'null_layout');
            break;
        }
    }
	/**
	...
	public function change_album_coverOp(){
		 ...
		 */
		$param = array();
		$param['field']		= array('apic_id','clic_id');
		$param['value']		= array(intval($_GET['id']),$_SESSION['clic_id']);
		$pic_info			= $model_album->getOnePicById($param);
		$return = $model_album->checkAlbum(array('album_aclass.clic_id'=>$_SESSION['clic_id'],'album_aclass.aclass_id'=>$pic_info['aclass_id']));
		if($return){
	...
	public function album_pic_infoOp(){
		 ...
		 * 验证
		 */
		$return = $model_album->checkAlbum(array('album_pic.clic_id'=>$_SESSION['clic_id'],'album_pic.apic_id'=>intval($_GET['id'])));
		if(!$return){
			showMessage(Language::get('album_parameter_error'),'','html','error');
		}
		...
		$param = array();
		$param['field']		= array('aclass_id','clic_id');
		$param['value']		= array(intval($_GET['class_id']),$_SESSION['clic_id']);
		$class_info			= $model_album->getOneClass($param);
		Tpl::output('class_info',$class_info);
		...
		Tpl::showpage('clic_album.pic_info');
	}
	...
	public function album_ad_ajaxOp(){
		...
		Tpl::showpage('clic_album.pic_scroll_ajax','null_layout');
	}
	...
	public function album_pic_delOp(){
		...
		$return = $model_album->checkAlbum(array('album_pic.clic_id'=>$_SESSION['clic_id'],'in_apic_id'=>$id));
		if(!$return){
			showDialog(Language::get('album_class_pic_del_lose'));
		}

		//删除图片
		$return = $model_album->delPic($id, $_SESSION['clic_id']);
		if($return){
	...
	public function album_pic_moveOp(){
		...
		Tpl::output('class_list',$class_list);
		Tpl::showpage('clic_album.move','null_layout');
	}
    ...
    public function replace_image_uploadOp() {
        ...
        $param = array();
        $param['field'] = array('apic_id', 'clic_id');
        $param['value'] = array($id, $_SESSION['clic_id']);
        ...
        $upload->set('default_dir', ATTACH_doctorS . DS . $_SESSION['clic_id'] . DS . $pic_cover);
         ...
         * 取得图像大小
         */
        list($width, $height, $type, $attr) = getimagesize(BASE_UPLOAD_PATH . DS . ATTACH_doctorS . DS . $_SESSION['clic_id'] . DS . $apic_info['apic_cover']);
        /**
         * 更新图片分类
         */
        $param = array();
	...
	public function album_pic_watermarkOp(){
		...
		$model_clic_wm = Model('clic_watermark');
		$clic_wm_info = $model_clic_wm->getOneclicWMByclicId($_SESSION['clic_id']);
		if ($clic_wm_info['wm_image_name'] == '' && $clic_wm_info['wm_text'] == ''){
			showDialog(Language::get('album_class_setting_wm'),"index.php?act=clic_album&op=clic_watermark",'error','CUR_DIALOG.close();');//"请先设置水印"
		}
		import('libraries.gdimage');
		$gd_image = new GdImage();
		$gd_image->setWatermark($clic_wm_info);

		foreach ($wm_list as $v) {
			$gd_image->create(BASE_UPLOAD_PATH.DS.ATTACH_doctorS.DS.$_SESSION['clic_id'].DS.str_ireplace('.', '_1280.', $v['apic_cover']));//生成有水印的大图
		}
	 ...
	 * 水印管理
	 */
	public function clic_watermarkOp(){
		/**
		 * 读取语言包
		 */
		...
		if (chksubmit()){
			...
			if (!empty($_FILES['image']['name'])){
				...
				if ($result){
					 ...
					 * 删除旧水印
					 */
					if (!empty($clic_wm_info['wm_image_name'])){
						@unlink(BASE_UPLOAD_PATH.DS.ATTACH_WATERMARK.DS.$clic_wm_info['wm_image_name']);
					}
				}else {
			...
			$param['wm_id'] = $clic_wm_info['wm_id'];
			$result = $model_clic_wm->updateclicWM($param);
			if ($result){
				showDialog(Language::get('clic_watermark_congfig_success'),'reload','succ');
			}else {
				showDialog(Language::get('clic_watermark_congfig_fail'));
			}
		}
		/**
		 * 获取水印字体
		...
		if (empty($clic_wm_info)){
			/**
			 * 新建店铺水印设置信息
			 */
			$model_clic_wm->addclicWM(array(
		...
		Tpl::output('clic_wm_info',$clic_wm_info);
		Tpl::showpage('clic_watermark.form');
	}    /**
     * 上传图片
     *
     */
    public function image_uploadOp() {
        ...
        $album_count = Model('album')->getCount(array('clic_id' => $clic_id));
         ...
         */
        $upload = new UploadFile();
        $upload->set('default_dir', ATTACH_doctorS . DS . $clic_id . DS . $upload->getSysSetPath());
        $upload->set('max_size', C('image_max_filesize'));
        $upload->set('thumb_width', doctorS_IMAGES_WIDTH);
        $upload->set('thumb_height', doctorS_IMAGES_HEIGHT);
        $upload->set('thumb_ext', doctorS_IMAGES_EXT);
        $upload->set('fprefix', $clic_id);
        ...
        list($width, $height, $type, $attr) = getimagesize(BASE_UPLOAD_PATH . DS . ATTACH_doctorS . DS . $clic_id . DS . $pic);
	...
	private function profile_menu($menu_type,$menu_key=''){
		...
		switch ($menu_type) {
			case 'album':
				$menu_array	= array(
				1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?act=clic_album'),
				2=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?act=clic_album&op=clic_watermark')
				);
				break;
			case 'album_pic':
				$menu_array	= array(
				1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?act=clic_album'),
				2=>array('menu_key'=>'pic_list','menu_name'=>Language::get('nc_member_path_album_pic_list'),'menu_url'=>'index.php?act=clic_album&op=album_pic_list&id='.intval($_GET['id'])),
				3=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?act=clic_album&op=clic_watermark')
				);
				break;
			case 'album_pic_info':
				$menu_array	= array(
				1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?act=clic_album'),
				2=>array('menu_key'=>'pic_info','menu_name'=>Language::get('nc_member_path_album_pic_info'),'menu_url'=>'index.php?act=clic_album&op=album_pic_info&id='.intval($_GET['id']).'&class_id='.intval($_GET['class_id'])),
				3=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?act=clic_album&op=clic_watermark')
				);
				break;
		}
	...
	public function ajax_change_imgmessageOp(){
		 ...
		 * 小图尺寸
		 */
		list($width, $height, $type, $attr) = getimagesize(BASE_UPLOAD_PATH.DS.ATTACH_doctorS.DS.$_SESSION['clic_id'].DS.str_ireplace('.', '_small.', $pic_info['0']['apic_cover']));
		if(strtoupper(CHARSET) == 'GBK'){
	...
	public function ajax_check_class_nameOp(){
		...
		$model_album	= Model('album');
		$param = array();
		$param['field']		= array('aclass_name','clic_id');
		$param['value']		= array($ac_name,$_SESSION['clic_id']);
		$class_info = $model_album->getOneClass($param);
		if(!empty($class_info)){
			echo 'false';die;
		}else{
