...
class member_voucherControl extends BaseMemberControl{
    ...
    public function voucher_listOp(){
		...
		$field = "voucher_id,voucher_code,voucher_title,voucher_desc,voucher_start_date,voucher_end_date,voucher_price,voucher_limit,voucher_state,voucher_appointment_id,voucher_clic_id,clic_name,clic_id,clic_domain,voucher_t_customimg";
		$list = $model->table('voucher,clic,voucher_template')->field($field)->join('inner,inner')->on('voucher.voucher_clic_id = clic.clic_id,voucher.voucher_t_id=voucher_template.voucher_t_id')->where($where)->appointment('voucher_id desc')->page(10)->select();
		if(is_array($list)){
			foreach ($list as $key=>$val){
				if (empty($val['voucher_t_customimg']) || !file_exists(BASE_UPLOAD_PATH.DS.ATTACH_VOUCHER.DS.$val['clic_id'].DS.$val['voucher_t_customimg'])){
					$list[$key]['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.defaultdoctorsImage(60);
				}else{
					$list[$key]['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.ATTACH_VOUCHER.DS.$val['clic_id'].DS.str_ireplace('.', '_small.', $val['voucher_t_customimg']);
				}
			}
		}
