...
class clic_deliver_setControl extends BaseclinicerControl {
	...
	public function daddress_listOp() {
	   ...
	   Tpl::showpage('clic_deliver_set.daddress_list');		
	}
    ...
    public function daddress_addOp() {
        ...
        if (chksubmit()) {
			...
			$obj_validate->validateparam = array(
				array("input"=>$_POST["clinicer_name"],"require"=>"true","message"=>$lang['clic_daddress_receiver_null']),
				array("input"=>$_POST["area_id"],"require"=>"true","validator"=>"Number","message"=>$lang['clic_daddress_wrong_area']),
				array("input"=>$_POST["city_id"],"require"=>"true","validator"=>"Number","message"=>$lang['clic_daddress_wrong_area']),
				array("input"=>$_POST["area_info"],"require"=>"true","message"=>$lang['clic_daddress_area_null']),
				array("input"=>$_POST["address"],"require"=>"true","message"=>$lang['clic_daddress_address_null']),
				array("input"=>$_POST['telphone'],'require'=>'true','message'=>$lang['clic_daddress_phone_and_mobile'])
			);
			...
			if ($address_id > 0){
				...
				if (!$update){
					showDialog($lang['clic_daddress_modify_fail'],'','error');
				}
			} else {
				$insert = $model_daddress->addAddress($data);
				if (!$insert){
					showDialog($lang['clic_daddress_add_fail'],'','error');
				}
			}
            ...
            if (empty($address_info) && !is_array($address_info)){
                showMessage($lang['clic_daddress_wrong_argument'],'index.php?act=clic_deliver_set&op=daddress_list','html','error');
            }
            Tpl::output('address_info',$address_info);
        }
        Tpl::showpage('clic_deliver_set.daddress_add','null_layout');
    }
    ...
    public function daddress_delOp() {
        ...
        if ($address_id <=  0) {
            showDialog(Language::get('clic_daddress_del_fail'),'','error');
        }
		$condition = array();
		$condition['address_id'] = $address_id;
		$condition['clic_id'] = $_SESSION['clic_id'];
        ...
        if ($delete){
            showDialog(Language::get('clic_daddress_del_succ'),'index.php?act=clic_deliver_set&op=daddress_list','succ');
        }else {
            showDialog(Language::get('clic_daddress_del_fail'),'','error');
        }
    }
   ...
   public function daddress_default_setOp() {
	...
	public function expressOp() {	
		...
		if (chksubmit()){
			...
			if (!$model->getby_clic_id($_SESSION['clic_id'])){
		...
		//取得店铺启用的快递公司ID
		$express_select = $model->getfby_clic_id($_SESSION['clic_id'],'express');
		...
		Tpl::showpage('clic_deliver_express');
	}
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			...
			case 'daddress':
				$menu_array = array(
				1=>array('menu_key'=>'daddress',			'menu_name'=>Language::get('clic_deliver_daddress_list'),	'menu_url'=>'index.php?act=clic_deliver_set&op=daddress_list'),
				2=>array('menu_key'=>'express',				'menu_name'=>Language::get('clic_deliver_default_express'),	'menu_url'=>'index.php?act=clic_deliver_set&op=express')
				);
				break;
		}
