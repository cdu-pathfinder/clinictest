...
class pointvoucherControl extends BaseHomeControl {
	...
	public function pointvoucherOp(){
		...
		$voucher = $model->table('voucher_template,clic')->field($field)->join('left')->on($on)->where(array('voucher_t_state'=>$this->templatestate_arr['usable'][0],'voucher_t_end_date'=>array('gt',time())))->select();
		if (!empty($voucher)){
	...
	public function voucherexchangeOp(){
		...
		if ($result){
			...
			$field = 'voucher_template.*,clic.clic_id,clic.clic_label,clic.clic_name,clic.clic_domain';
			$on = 'voucher_template.voucher_t_clic_id=clic.clic_id';
			$voucher_info = $model->table('voucher_template,clic')->field($field)->join('left')->on($on)->where(array('voucher_t_id'=>$vid,'voucher_t_state'=>$this->templatestate_arr['usable'][0],'voucher_t_end_date'=>array('gt',time())))->find();
			if (empty($voucher_info)){
				$result = false;
				$message = Language::get('home_voucher_recappointmentror');
			}else {
				//查询会员信息
	...
	public function voucherexchange_saveOp(){
		...
		if (empty($voucher_info)){
			showDialog(Language::get('home_voucher_recappointmentror'),'','error',$js);
		}
		//验证是否为店铺自己
		if ($_SESSION['clic_id'] == $voucher_info['voucher_t_clic_id']){
			showDialog(Language::get('home_voucher_unableexchangself'),'','error',$js);
		}
		$voucher_list= $model->table('voucher')->where(array('voucher_owner_id'=>$_SESSION['member_id'],'voucher_clic_id'=>$voucher_info['voucher_t_clic_id'],'voucher_end_date'=>array('gt',time())))->select();
		...
		$insert_arr['voucher_clic_id'] = $voucher_info['voucher_t_clic_id'];
		...
		$points_arr['point_appointmentsn'] = $insert_arr['voucher_code'];
		$points_arr['pl_desc'] = Language::get('home_voucher').$insert_arr['voucher_code'].Language::get('points_pointappointmentdesc');
