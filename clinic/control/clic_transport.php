...
class clic_transportControl extends BaseclinicerControl {
	...
	public function listOp(){
		...
		$list = $model_transport->getTransportList(array('clic_id'=>$_SESSION['clic_id']),4);
		...
		Tpl::showpage('clic_transport.list');
	}
	...
	public function addOp(){
		...
		self::profile_menu('transport','transport');
		Tpl::showpage('clic_transport.add');
	}
	...
	public function deleteOp(){
		...
		$model_transport = Model('transport');
		$transport = $model_transport->getTransportInfo(array('id'=>$id));
		if ($transport['clic_id'] != $_SESSION['clic_id']){
	...
	public function saveOp(){
		...
		$trans_info['clic_id'] 	= $_SESSION['clic_id'];
		...
		if ($result){
			header('location: index.php?act=clic_transport&type='.$_POST['type']);exit;
		}else{
			showMessage(Language::get('transport_op_fail'),$_SERVER['HTTP_REFERER'],'html','error');
		}
	}
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'transport':
				$menu_array = array(
				1=>array('menu_key'=>'transport',	'menu_name'=>Language::get('nc_member_path_postage'),		'menu_url'=>'index.php?act=clic_transport')
				);
				break;
		}
