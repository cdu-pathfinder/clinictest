...
class sns_albumControl extends BaseSNSControl {
	...
	public function album_cateOp(){
		...
		$where = array();	// 条件
		$where['member_id']	= $this->master_id;
		$appointment	= 'ac_sort asc';
		// 相册
		$ac_list = $model->table('sns_albumclass')->where($where)->appointment($appointment)->select();
		$count = 0; // 图片总数量
		if(!empty($ac_list)){
			// 相册中商品数量
	...
	public function album_pic_listOp(){
		...
		$appointment = 'ap_id desc';
		if($_GET['sort'] != ''){
			switch ($_GET['sort']){
				case '0':
					$appointment = 'upload_time desc';
					break;
				case '1':
					$appointment = 'upload_time asc';
					break;
				case '2':
					$appointment = 'ap_size desc';
					break;
				case '3':
					$appointment = 'ap_size asc';
					break;
				case '4':
					$appointment = 'ap_name desc';
					break;
				case '5':
					$appointment = 'ap_name asc';
					break;
			}
		}
		$pic_list = $model->table('sns_albumpic')->where($where)->appointment($appointment)->page(36)->select();
	...
	public function album_pic_infoOp(){
		...
		$where['member_id']	= $this->master_id;
		$each_num = 9;
		$pic_list			= $model->table('sns_albumpic')->where($where)->appointment('ap_id desc')->page($each_num)->select();
		if(empty($pic_list)){
			showMessage(Language::get('wrong_argument'), '', '', 'error');
		}
