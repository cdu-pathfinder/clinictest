...
class homeControl extends BaseMemberControl {
	...
	public function moreOp(){
		...
		// 用户标签列表
		$mtag_array = $model->table('sns_membertag')->appointment('mtag_sort asc')->limit(1000)->select();
	...
	public function showmsgbatchOp() {
		...
		$drop_type = trim($_GET['drop_type']);
		$referer_url = getReferer();
		if(!in_array($drop_type,array('msg_system','msg_clinicer')) || $message_id<=0){
			showMessage(Language::get('wrong_argument'),$referer_url,'html','error');
		}
		//查询站内信
		$param = array();
		...
		if ($drop_type == 'msg_clinicer'){
			//查询店铺信息
			$model_clic = Model('clic');
			$clic_info = $model_clic->getclicInfo(array('member_id'=>"{$message_info['from_member_id']}"));
			$message_info['from_member_name'] =  $clic_info['clic_name'];
			$message_info['clic_id'] =  $clic_info['clic_id'];
		}
	...
	public function dropbatchmsgOp() {
		...
		$message_id = trim($_GET['message_id']);
		$drop_type = trim($_GET['drop_type']);
		if(!in_array($drop_type,array('msg_system','msg_clinicer')) || empty($message_id)){
	 ...
	 * 重新申请开店
	 */
	public function clic_reapplyOp(){
		Language::read('member_home_member');
		Model()->table('clic')->update(array('clic_id'=>$_SESSION['clic_id'], 'clic_state'=>2));
