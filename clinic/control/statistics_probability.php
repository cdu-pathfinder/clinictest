...
class statistics_probabilityControl extends BaseclinicerControl {
	...
	public function probability_statisticsOp() {
		...
		$where = "salenum.date in (".$request_date_str.") and ".$flow_tablename.".clic_id = '".$_SESSION['clic_id']."' and salenum.clic_id = '".$_SESSION['clic_id']."' and ".$flow_tablename.".type = 'sum'";
		$group = 'salenum.date';
		...
		if($_GET['type'] == 'month'){
			...
			if($_GET['add_time_from'] != '' && $_GET['add_time_to'] != ''){
				...
				$to = $_GET['add_time_to'];
				$main_title = Language::get('clic_ps_search_result');
