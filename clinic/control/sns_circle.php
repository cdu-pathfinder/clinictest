...
class sns_circleControl extends BaseSNSControl {
	...
	public function themeOp(){
		$model = Model();
		$theme_list = $model->table('circle_theme')->where(array('member_id'=>$this->master_id))->page(10)->appointment('theme_id desc')->select();
	...
	public function circleOp(){
		$model = Model();
		$cm_list = $model->table('circle_member')->where(array('member_id'=>$this->master_id))->appointment('cm_jointime desc')->select();
		if(!empty($cm_list)){
