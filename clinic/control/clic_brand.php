...
class clic_brandControl extends BaseclinicerControl {
	public function __construct() {
		parent::__construct();
		Language::read('member_clic_brand');
		$lang	= Language::getLangContent();
		//卖家店铺是否审核的判断
		if($this->clic_info['clic_state'] == 2) {
			showMessage($lang['clic_auditing_tip'],'index.php?act=clic','html','error');
		}
	}
	...
	public function brand_listOp() {
		...
		$condition['clicid_equal']			= "{$_SESSION['clic_id']}";
	...
	public function brand_addOp() {
		...
		if($_GET['brand_id'] != '') {
			...
			if (empty($brand_array) || $brand_array['clic_id'] != $_SESSION['clic_id']){
		...
		// 一级商品分类
		$gc_list = Model()->table('doctors_class')->where(array('gc_parent_id'=>0))->select();
		Tpl::output('gc_list', $gc_list);
		
		Tpl::showpage('clic_brand.add','null_layout');
	}
	...
	public function brand_saveOp() {
		...
		if (chksubmit()) {
			...
			array("input"=>$_POST["brand_name"], "require"=>"true", "message"=>$lang['clic_doctors_brand_name_null'])
			);
			...
			if ($error != ''){
				...
				if ($result){
					showDialog($lang['clic_doctors_brand_apply_success'],'index.php?act=clic_brand&op=brand_list','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
				}else {
		...
		if ($_POST['form_submit'] == 'ok' and intval($_POST['brand_id']) != 0) {
			...
			if ($error != ''){
				...
				if ($result){
					...
					showDialog($lang['nc_common_save_succ'],'index.php?act=clic_brand&op=brand_list','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
				}else {
					showDialog($lang['nc_common_save_fail']);
				}
			}
	...
	public function drop_brandOp() {
		...
		if ($brand_id > 0){
			...
			unset($brand_array);
			showDialog(Language::get('nc_common_del_succ'),'index.php?act=clic_brand&op=brand_list','succ');
		}else {
			showDialog(Language::get('nc_common_del_fail'));
		}
	}
	...
	private function profile_menu($menu_type,$menu_key='',$array=array()) {
		...
		switch ($menu_type) {
			case 'brand_list':
				$menu_array = array(
				1=>array('menu_key'=>'brand_list',	'menu_name'=>$lang['nc_member_path_brand_list'],			'menu_url'=>'index.php?act=clic_brand&op=brand_list'));
				break;
		}
		if(!empty($array)) {
			$menu_array[] = $array;
