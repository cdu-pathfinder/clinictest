...
class indexControl extends BaseHomeControl{
	public function indexOp(){
        ...
        $xianshi_item = $model_xianshi_doctors->getXianshidoctorsCommendList(4);
	...
	public function josn_classOp() {
		...
		$doctors_class		= $model_class->getClassList(array('gc_parent_id'=>intval($_GET['gc_id']),'appointment'=>'gc_parent_id asc,gc_sort asc,gc_id asc'));
		$array				= array();
	...
	public function viewed_infoOp(){
		...
		$doctors_list = $model_doctors->getVieweddoctorsList();
		if(is_array($doctors_list) && !empty($doctors_list)) {
		    $viewed_doctors = array();
		    foreach ($doctors_list as $key => $val) {
		        $doctors_id = $val['doctors_id'];
		        $val['url'] = urlclinic('doctors', 'index', array('doctors_id' => $doctors_id));
		        $val['doctors_image'] = thumb($val, 60);
		        $viewed_doctors[$doctors_id] = $val;
		    }
		    $info['viewed_doctors'] = $viewed_doctors;
		}
		if (strtoupper(CHARSET) == 'GBK'){
