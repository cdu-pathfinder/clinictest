...
class statistics_flowControl extends BaseclinicerControl {
	public function __construct() {
		parent::__construct();
		Language::read('member_clic_statistics');
	}
	/**
	 * 流量统计
	 *
	...
	public function flow_statisticsOp() {
		//确定统计分表名称
		$flow_tableid = 0;
		$len = strlen(strval(intval($_SESSION['clic_id'])));
		$last_num = substr(strval(intval($_SESSION['clic_id'])), $len-1,1);
		switch ($last_num){
			case 1:
				$flow_tableid = 1;
				break;
		...
		if($_GET['type'] == 'month'){
			...
			$model = Model();
			$flow_array = $model->table($flow_tablename)->field('date,clicknum')->where(array('date'=>array('in',$request_date_str),'clic_id'=>$_SESSION['clic_id'],'type'=>'sum'))->select();
			//整理流量信息数组
			$result_clicknum_str = '';
			...
			if($_GET['add_time_from'] != '' && $_GET['add_time_to'] != ''){
				...
				$to = $_GET['add_time_to'];
				$main_title = Language::get('clic_flow_search_result');
	 ...
	 * @return 
	 */
	public function doctors_flow_statisticsOp(){
		//确定统计分表名称
		$flow_tableid = 0;
		$len = strlen(strval(intval($_SESSION['clic_id'])));
		$last_num = substr(strval(intval($_SESSION['clic_id'])), $len-1,1);
		...
		if($_GET['type'] == 'month'){
			...
			if($_GET['add_time_from'] != '' && $_GET['add_time_to'] != ''){
				...
				$from = $_GET['add_time_from'];
				$to = $_GET['add_time_to'];
				$main_title = Language::get('clic_flow_rank_search_result');
				$sub_title  = substr($from,0,4).'.'.substr($from,4,2).'.'.substr($from,6,2).'-'.substr($to,0,4).'.'.substr($to,4,2).'.'.substr($to,6,2);
			}else{
				//默认显示本周商品流量排名
				$day = date('l',time());
		...
		$model = Model();
		$table = $flow_tablename.',doctors';
		$field = 'sum('.$flow_tablename.'.clicknum) as sum,'.$flow_tablename.'.doctors_id,doctors.doctors_name';
		$where = $flow_tablename.".type = 'doctors' and ".$flow_tablename.".date in (".$request_date_str.") and ".$flow_tablename.".clic_id = '".$_SESSION['clic_id']."'";
		$group = $flow_tablename.'.doctors_id';
		$on    = 'doctors.doctors_id='.$flow_tablename.'.doctors_id';
		$appointment = 'sum desc';
		$limit = '10';
		$flow_array = $model->table($table)->field($field)->join('left')->on($on)->where($where)->group($group)->limit($limit)->appointment($appointment)->select();
		//处理数组信息
		$result_doctors_str = '';
		$result_clicknum_str = '';
		if(!empty($flow_array)){
			foreach ($flow_array as $k=>$v){
				$result_doctors_str .= "'".addslashes(html_entity_decode($v['doctors_name']))."',";//还原被转换的双引号，加斜线防止中间有单引号使页面的JS数组出错
				$result_clicknum_str .= $v['sum'].',';
			}
		}
		$result_doctors_str = trim($result_doctors_str,',');
		$result_clicknum_str = trim($result_clicknum_str,',');
		//模版输出
		Tpl::output('result_doctors_str',$result_doctors_str);
		Tpl::output('result_clicknum_str',$result_clicknum_str);
		Tpl::output('main_title',$main_title);
		Tpl::output('sub_title',$sub_title);
		self::profile_menu('doctors_flow');
		Tpl::output('menu_sign','flow_statistics');
		Tpl::output('menu_sign_url','index.php?act=statistics&op=flow_statistics');
		Tpl::output('menu_sign1','doctors_flow');
		Tpl::showpage('doctors_flow');
	}
	...
	private function profile_menu($menu_key='') {
        $menu_array	= array(
            1=>array('menu_key'=>'clic_flow','menu_name'=>Language::get('stat_clic_flow'),	'menu_url'=>'index.php?act=statistics_flow&op=flow_statistics'),
            2=>array('menu_key'=>'doctors_flow','menu_name'=>Language::get('stat_doctors_flow'),	'menu_url'=>'index.php?act=statistics_flow&op=doctors_flow_statistics')
        );
