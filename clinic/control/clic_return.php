...
class clic_returnControl extends BaseclinicerControl {
	...
	public function indexOp() {
		...
		self::profile_menu('return',$appointment_lock);
		Tpl::showpage('clic_return');
	}
	/**
	 * 退货审核页
	 *
	...
	public function editOp() {
		...
		if (chksubmit()) {
			if ($return['clinicer_state'] != '1') {//检查状态,防止页面刷新不及时造成数据错误
				showDialog(Language::get('wrong_argument'),'reload','error','CUR_DIALOG.close();');
			}
			$appointment_id = $return['appointment_id'];
			$refund_array = array();
			...
			if ($refund_array['clinicer_state'] == '2' && empty($_POST['return_type'])) {
			    $refund_array['return_type'] = '2';//退货类型:1为不用退货,2为需要退货
			} elseif ($refund_array['clinicer_state'] == '3') {
			...
			if ($state) {
    			if ($refund_array['clinicer_state'] == '3' && $return['appointment_lock'] == '2') {
    			    $model_refund->editappointmentUnlock($appointment_id);//订单解锁
    			}
    			$this->recordclinicerLog('退货处理，退货编号：'.$return['refund_sn']);
		...
		Tpl::showpage('clic_return_edit','null_layout');
	}
	/**
	 * 收货
	 *
	...
	public function receiveOp() {
		...
		if (chksubmit()) {
			if ($return['clinicer_state'] != '2' || $return['doctors_state'] != '2') {//检查状态,防止页面刷新不及时造成数据错误
				showDialog(Language::get('wrong_argument'),'reload','error','CUR_DIALOG.close();');
			}
			$refund_array = array();
			if ($_POST['return_type'] == '3' && $delay_time > 0) {
			    $refund_array['doctors_state'] = '3';
			} else {
    			$refund_array['receive_time'] = time();
    			$refund_array['receive_message'] = '确认收货完成';
    			$refund_array['refund_state'] = '2';//状态:1为处理中,2为待管理员处理,3为已完成
    			$refund_array['doctors_state'] = '4';
			}
			$state = $model_refund->editRefundReturn($condition, $refund_array);
			if ($state) {
			    $this->recordclinicerLog('退货确认收货，退货编号：'.$return['refund_sn']);
				showDialog(Language::get('nc_common_save_succ'),'reload','succ','CUR_DIALOG.close();');
			} else {
				showDialog(Language::get('nc_common_save_fail'),'reload','error','CUR_DIALOG.close();');
			}
		...
		Tpl::showpage('clic_return_receive','null_layout');
	}
	/**
	 * 退货记录查看页
	 *
	...
	public function viewOp() {
		...
		Tpl::showpage('clic_return_view','null_layout');
	}
	/**
	 * 用户中心右边，小导航
	 *
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'return':
				$menu_array = array(
					array('menu_key'=>'2','menu_name'=>'售前退货',	'menu_url'=>'index.php?act=clic_return&lock=2'),
					array('menu_key'=>'1','menu_name'=>'售后退货','menu_url'=>'index.php?act=clic_return&lock=1')
				);
				break;
		}
