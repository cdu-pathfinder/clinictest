...
class member_returnControl extends BaseMemberControl {
	...
	public function indexOp(){
		...
		self::profile_menu('member_appointment','buyer_return');
	...
	public function shipOp(){
		...
		if (chksubmit()) {
			if ($return['clinicer_state'] != '2' || $return['doctors_state'] != '1') {//检查状态,防止页面刷新不及时造成数据错误
	...
	public function delayOp(){
		...
		Tpl::output('return',$return);
		if (chksubmit()) {
			if ($return['clinicer_state'] != '2' || $return['doctors_state'] != '3') {//检查状态,防止页面刷新不及时造成数据错误
				showDialog(Language::get('wrong_argument'),'reload','error','CUR_DIALOG.close();');
			}
			$refund_array = array();
			$refund_array['delay_time'] = time();
