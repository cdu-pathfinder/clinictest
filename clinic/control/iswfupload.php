...
class iswfuploadControl {
	...
	public function import_pic_uploadOp() {
		...
		if(empty($upload_list) or !is_array($upload_list)){
			echo json_encode(array('state'=>'false','message'=>$lang['iswfupload_image'].$_POST['Filename'].$lang['iswfupload_not_doctors_image']));
			exit;
		} 
		 ...
		 * 取得图像大小
		 */
		list($width, $height, $type, $attr) = getimagesize(UPLOAD_SITE_URL . '/' . ATTACH_doctorS . '/' .$clic_id.DS.$upload->getSysSetPath().$upload->file_name);
		
		if (C('ftp_open') && C('thumb.save_type')==3){
			import('function.ftp');
			$img_path = $upload->getSysSetPath().$upload->file_name;
			if ($_url = remote_ftp(ATTACH_doctorS.DS.$clic_id,$img_path,false)){
				$image_cover = $_url.'/';
			}
		}		
