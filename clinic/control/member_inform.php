...
class member_informControl extends BaseMemberControl{
    ...
    public function inform_submitOp() {
        ...
        //获取商品详细信息
        $doctors_info = $this->get_doctors_info_byid($doctors_id);
        ...
        $model_inform = Model('inform');
        //检查是否当前正在举报
        if($model_inform->isProcessOfInform($doctors_id)) {
    ...
    public function inform_saveOp() {
        ...
        $doctors_id = intval($_POST['inform_doctors_id']);
        ...
        $input['inform_doctors_id'] = $doctors_id;
        $input['inform_doctors_name'] = $doctors_info['doctors_name'];
        ...
        $input['inform_clic_id'] = $doctors_info['clic_id'];
     ...
     * 根据id获取商品详细信息
     */
    private function get_doctors_info_byid($doctors_id) {
        ...
        $doctors_condition = array();
        $doctors_condition['doctors_id'] = $doctors_id;
        $doctors_info = $model_doctors->getdoctorsOnlineInfo($doctors_condition);

        //检查该商品是否存在
        if(empty($doctors_info)) {
            showMessage(Language::get('doctors_null'),'','html','error');
        }
        return $doctors_info;
    }
